...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* v1 pre-delivery: client signature block removed per user request — only
     seller signature remains. One-column grid + max-width keeps the single
     block at half the invoice width (same footprint as the previous two-
     column layout's left cell). */
  .signatures { display:grid; grid-template-columns:1fr; gap:32px; margin-bottom:32px; max-width:50%; }
