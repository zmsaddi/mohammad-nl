...
export async function getDistributablePoolForPeriod(startDate, endDate) {
  ...
  // 5. Already distributed for this exact period
  const { rows: r5 } = await sql`
    SELECT COALESCE(SUM(amount), 0) AS v FROM profit_distributions
    WHERE ((${f}::text IS NULL AND base_period_start IS NULL) OR base_period_start = ${f})
      AND ((${t}::text IS NULL AND base_period_end IS NULL) OR base_period_end = ${t})
  `;
...
export async function addProfitDistribution(data) {
  ...
  return withTx(async (client) => {
    ...
    // 3e. Already distributed for this exact period
    const { rows: distRows } = await client.sql`
      SELECT COALESCE(SUM(amount), 0) AS already FROM profit_distributions
      WHERE ((${startDate}::text IS NULL AND base_period_start IS NULL) OR base_period_start = ${startDate})
        AND ((${endDate}::text IS NULL AND base_period_end IS NULL) OR base_period_end = ${endDate})
    `;
