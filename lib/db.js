...
export async function addPurchase(data) {
  ...
  const { rows } = await sql`
    ...
    VALUES (${data.date}, ${data.supplier}, ${data.item}, ${data.quantity}, ${data.unitPrice}, ${total}, ${data.paymentType || 'كاش'}, ${refCode}, ${data.createdBy || ''}, ${data.notes || ''})
...
export async function updateDelivery(data) {
  ...
  // Prevent double confirmation: use oldStatus (read BEFORE update), not current DB state
  if (oldStatus === 'تم التوصيل' && data.status === 'تم التوصيل') return;
...
export async function getSummaryData(from, to) {
  ...
  const totalCreditSales = sales.filter((s) => s.payment_type === 'آجل').reduce((s, r) => s + (r.total || 0), 0);
  const totalPaidAtSale = sales.filter((s) => s.payment_type === 'آجل').reduce((s, r) => s + (r.paid_amount || 0), 0);
