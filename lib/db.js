  ...
  if (role === 'manager') {
    const { rows } = await sql`
      SELECT b.id, b.owner_username, b.type, b.balance, b.active,
             u.name AS owner_name, u.role AS owner_role
      FROM cash_boxes b LEFT JOIN users u ON u.username = b.owner_username
      WHERE b.owner_username = ${username} OR u.role = 'driver'
  ...
  if (role === 'driver') {
    const { rows } = await sql`
      SELECT b.id, b.owner_username, b.type, b.balance, b.active,
             u.name AS owner_name, u.role AS owner_role
      FROM cash_boxes b LEFT JOIN users u ON u.username = b.owner_username
      WHERE b.owner_username = ${username}`;
    return rows;
  }
  return [];
}
...
export async function updateUser(data) {
  ...
  // Treasury (Phase 1): if a role change promotes the user into a cash-handling
  // role, ensure they have a custody box. Best-effort, never breaks the edit.
  if (['admin', 'manager', 'driver'].includes(data.role)) {
    try {
      const { rows } = await sql`SELECT username FROM users WHERE id = ${data.id}`;
      if (rows[0]?.username) await ensureCashBox(rows[0].username, data.role);
