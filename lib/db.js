...
export async function addPurchase(data) {
  ...
  return withTx(async (client) => {
    ...
    if (paidAmount > 0.005) {
      const { rows: dpRows } = await client.sql`
        ...
        RETURNING id
      `;
      // Treasury shadow (no-op unless TREASURY_ENABLED): the down payment debits
      // the paying user's (purchase creator's) custody box — mirrors paySupplier.
      // Previously MISSING, so registering a purchase never reduced the buyer's box.
      await emitCashMovement(client, {
        ownerUsername: data.createdBy,
        signedAmount: -paidAmount,
        method: data.paymentType === 'بنك' ? 'بنك' : 'كاش',
        kind: 'supplier_payment',
        sourceTable: 'supplier_payments',
        sourceId: dpRows[0].id,
...
export async function deletePurchase(id) {
  return withTx(async (client) => {
    ...
    // Treasury: a hard delete means "this purchase never happened" — reverse the
    // box debits of every supplier payment before the rows are removed, so the
    // buyer's custody box is credited back exactly (no-op unless TREASURY_ENABLED).
    const { rows: pays } = await client.sql`SELECT id FROM supplier_payments WHERE purchase_id = ${id}`;
    for (const pay of pays) {
      await reverseCashMovements(client, 'supplier_payments', pay.id);
    }
...
export async function paySupplier(data) {
  ...
  return withTx(async (client) => {
    ...
    const { rows: paymentRows } = await client.sql`
      ...
      RETURNING id
    `;
    // Treasury shadow (no-op unless TREASURY_ENABLED): supplier payment debits
    // the paying user's (purchase creator's) box.
    await emitCashMovement(client, {
      ownerUsername: data.createdBy,
      signedAmount: -amt,
      method: data.paymentMethod === 'بنك' ? 'بنك' : 'كاش',
      kind: 'supplier_payment',
      sourceTable: 'supplier_payments',
      sourceId: paymentRows[0].id,
      createdBy: data.createdBy || '',
    });
    return {
...
export async function getSales(clientName) {
  ...
  // Phase 2 (order lifecycle): also surface the linked delivery's status +
  // assigned driver via scalar subqueries (one row guaranteed, no JOIN
  // row-multiplication) so the sales list can show the real delivery stage
  // without a second fetch. NULL when a sale has no delivery yet.
  if (clientName) {
    const { rows } = await sql`
      SELECT s.*, i.ref_code AS invoice_ref_code,
        (SELECT d.status FROM deliveries d WHERE d.sale_id = s.id ORDER BY d.id DESC LIMIT 1) AS delivery_status,
        (SELECT d.assigned_driver FROM deliveries d WHERE d.sale_id = s.id ORDER BY d.id DESC LIMIT 1) AS delivery_driver,
        (SELECT d.address FROM deliveries d WHERE d.sale_id = s.id ORDER BY d.id DESC LIMIT 1) AS delivery_address
      FROM sales s
...
export async function cancelSale(client, options) {
  ...
  for (const payment of paymentRows) {
    ...
    const { rows: refRows } = await client.sql`
      ...
      RETURNING id
    `;
    // Treasury shadow (no-op unless TREASURY_ENABLED): the refund debits the
    // canceller's box (reverses the collection; nets out at handover).
    await emitCashMovement(client, {
      ownerUsername: cancelledBy,
      signedAmount: negativeAmount,
      method: payment.payment_method === 'بنك' ? 'بنك' : 'كاش',
      kind: 'refund',
      sourceTable: 'payments',
      sourceId: refRows[0].id,
...
export async function addExpense(data) {
  ...
  const method = validPay.includes(data.paymentType) ? data.paymentType : 'كاش';
  // Wrapped in withTx so the treasury shadow write is atomic with the expense.
  return withTx(async (client) => {
    const { rows } = await client.sql`
      INSERT INTO expenses (date, category, description, amount, payment_type, created_by, notes)
      VALUES (${data.date}, ${data.category}, ${data.description}, ${data.amount}, ${method}, ${data.createdBy || ''}, ${data.notes || ''})
      RETURNING id
    `;
    // Treasury shadow (no-op unless TREASURY_ENABLED): expense debits the creator's box.
    await emitCashMovement(client, {
      ownerUsername: data.createdBy,
      signedAmount: -(parseFloat(data.amount) || 0),
      method,
      kind: 'expense',
...
export async function deleteExpense(id) {
  // Wrapped in withTx so the treasury reversal is atomic with the delete.
  return withTx(async (client) => {
    // SP-003: verify the expense exists before deleting
    const { rows } = await client.sql`SELECT id FROM expenses WHERE id = ${id}`;
    if (!rows.length) throw new Error('المصروف غير موجود');
    // Treasury: credit the creator's box back (reverse the expense debit) before
    // removing the row — no-op unless TREASURY_ENABLED.
    await reverseCashMovements(client, 'expenses', id);
...
export async function addPayment(data) {
  ...
  // Manual payment with no sale. Per the agreed model this IS a cash event:
  // the recorded amount enters the creator's box. Wrapped in withTx so the
  // treasury shadow write is atomic with the insert (no-op unless enabled).
  return withTx(async (client) => {
    const { rows } = await client.sql`
 ...
 */
// ── Treasury (Phase 1, P1b) — shadow cash movements ────────────────────────
// Records a movement into the new treasury ledger and updates the target box
// balance, IN THE SAME transaction as the money operation. STRICT no-op unless
// the TREASURY_ENABLED env flag is 'true', so any money flow may call it
// safely; nothing in the app reads these tables yet (shadow phase — displayed
// numbers are unchanged). Idempotent via a pre-check on
// (source_table, source_id, kind, box_id). Box resolution order: explicit
// boxId → the owner's custody box → the shared general box.
// Attribution rule (from locked decisions, not assumed): a person's own
// in/out movement hits THEIR box; commission/distribution/capital hit the
// general box. method (كاش/بنك) is only a tag.
// Treasury master switch. ON if the env override is set OR the admin enabled
// it via the settings toggle (key 'treasury_enabled') — so it can be flipped
// from the UI without a redeploy. Reading is failsafe (off on any error).
// Tests set the env var, which short-circuits before any DB read.
export async function isTreasuryEnabled(client) {
  if (process.env.TREASURY_ENABLED === 'true') return true;
  try {
    // Call .sql DIRECTLY on the owner — @vercel/postgres' transaction client
    // exposes `sql` as a class method that needs `this === client`. Extracting
    // it to a variable and calling it loses `this` and throws, which the catch
    // would silently turn into "disabled" — wedging the whole treasury on.
    const { rows } = (client && client.sql)
      ? await client.sql`SELECT value FROM settings WHERE key = 'treasury_enabled'`
      : await sql`SELECT value FROM settings WHERE key = 'treasury_enabled'`;
    return rows[0]?.value === 'true';
  } catch { return false; }
}

// Exported so tests can inject a mocked client (same pattern as
// calculateBonusInTx) and verify the flag/box/idempotency/balance logic
// without a real database.
export async function emitCashMovement(client, opts) {
  if (!(await isTreasuryEnabled(client))) return;
  const { kind, sourceTable, sourceId } = opts;
  if (!sourceId) return; // need a stable source row for idempotency
  // Guard: if the flag is enabled before /api/init created the treasury tables,
  // skip silently rather than 500 inside a real money-moving transaction.
  const { rows: tbl } = await client.sql`SELECT to_regclass('cash_boxes') AS t`;
  if (!tbl[0]?.t) return;
  let boxId = opts.boxId || null;
  if (!boxId && opts.ownerUsername) {
    const { rows } = await client.sql`SELECT id FROM cash_boxes WHERE owner_username = ${opts.ownerUsername}`;
    boxId = rows[0]?.id || null;
  }
  if (!boxId && opts.general) {
    const { rows } = await client.sql`SELECT id FROM cash_boxes WHERE type = 'main' LIMIT 1`;
    boxId = rows[0]?.id || null;
  }
  if (!boxId) return; // no box for this actor → skip (reconciliation will show it)
  const { rows: dup } = await client.sql`
    SELECT 1 FROM cash_movements
    WHERE source_table = ${sourceTable} AND source_id = ${sourceId}
      AND kind = ${kind} AND box_id = ${boxId}
    LIMIT 1
  `;
  if (dup.length) return; // already emitted — idempotent
  const amount = Math.round((parseFloat(opts.signedAmount) || 0) * 100) / 100;
  await client.sql`
    INSERT INTO cash_movements (
      date, box_id, counterparty_box_id, signed_amount, method, kind,
      source_table, source_id, created_by, notes
    ) VALUES (
      CURRENT_DATE, ${boxId}, ${opts.counterpartyBoxId || null}, ${amount},
      ${opts.method || 'كاش'}, ${kind}, ${sourceTable}, ${sourceId},
      ${opts.createdBy || ''}, ${opts.notes || ''}
    )
  `;
  await client.sql`UPDATE cash_boxes SET balance = balance + ${amount} WHERE id = ${boxId}`;
}

// Reverse + remove the shadow cash movements of a HARD-deleted source row.
// Used when a source (a purchase's supplier_payments, an expense) is deleted
// outright — the source no longer exists, so its shadow movements are removed
// and their exact balance effect is undone. Money-safe: it subtracts precisely
// what was added, and no-ops when the treasury is off or the table is absent.
// (Cancellations keep an audit trail instead — they INSERT reversing rows.)
export async function reverseCashMovements(client, sourceTable, sourceId) {
  if (!(await isTreasuryEnabled(client))) return;
  if (!sourceId) return;
  const { rows: tbl } = await client.sql`SELECT to_regclass('cash_movements') AS t`;
  if (!tbl[0]?.t) return;
  const { rows: movs } = await client.sql`
    SELECT box_id, signed_amount FROM cash_movements
    WHERE source_table = ${sourceTable} AND source_id = ${sourceId}
  `;
  for (const m of movs) {
    await client.sql`UPDATE cash_boxes SET balance = balance - ${m.signed_amount} WHERE id = ${m.box_id}`;
  }
  await client.sql`DELETE FROM cash_movements WHERE source_table = ${sourceTable} AND source_id = ${sourceId}`;
}

// Set/replace a box's OPENING balance — the physical cash count at go-live
// ("clean start from today"). Records ONE kind='opening' movement per box and
// brings the box balance to match. Re-entry REPLACES the prior opening (adjusts
// the balance by the delta only) so a miscount can be corrected without
// double-counting. Requires the treasury to be enabled. Amount must be ≥ 0.
export async function setOpeningBalance(boxId, amount, by) {
  const amt = Math.round((parseFloat(amount) || 0) * 100) / 100;
  return withTx(async (client) => {
    if (!(await isTreasuryEnabled(client))) throw new Error('نظام الصناديق غير مفعّل');
    if (amt < 0) throw new Error('الرصيد الافتتاحي لا يمكن أن يكون سالباً');
    const { rows: b } = await client.sql`SELECT id FROM cash_boxes WHERE id = ${boxId} FOR UPDATE`;
    if (!b.length) throw new Error('الصندوق غير موجود');
    const { rows: ex } = await client.sql`
      SELECT id, signed_amount FROM cash_movements WHERE box_id = ${boxId} AND kind = 'opening' ORDER BY id LIMIT 1`;
    if (ex.length) {
      const prev = parseFloat(ex[0].signed_amount) || 0;
      const delta = Math.round((amt - prev) * 100) / 100;
      await client.sql`UPDATE cash_movements SET signed_amount = ${amt}, date = CURRENT_DATE, created_by = ${by || ''}, notes = 'رصيد افتتاحي (مُعدّل)' WHERE id = ${ex[0].id}`;
      if (delta !== 0) await client.sql`UPDATE cash_boxes SET balance = balance + ${delta} WHERE id = ${boxId}`;
    } else {
      await client.sql`
        INSERT INTO cash_movements (date, box_id, signed_amount, method, kind, source_table, source_id, created_by, notes)
        VALUES (CURRENT_DATE, ${boxId}, ${amt}, 'كاش', 'opening', 'opening', ${boxId}, ${by || ''}, 'رصيد افتتاحي')`;
      await client.sql`UPDATE cash_boxes SET balance = balance + ${amt} WHERE id = ${boxId}`;
    }
    const { rows: nb } = await client.sql`SELECT balance FROM cash_boxes WHERE id = ${boxId}`;
    return { success: true, balance: parseFloat(nb[0]?.balance) || 0 };
  });
}

// Treasury reconciliation — the safety net that surfaces any money flow the box
// layer failed to record. Two checks:
//   integrityDiff = Σ(box balances) − Σ(all cash_movements)   → MUST be 0
//     (every balance change goes through a movement; non-zero = a real bug).
//   coverage.diff = Σ(non-opening movements) − net cash computed INDEPENDENTLY
//     from the core ledgers since the cutoff (the earliest opening date)  → ~0
//     when every flow emitted. NOTE: the core side filters on the business
//     `date`, so a post-go-live entry BACK-DATED before the cutoff can show a
//     non-zero coverage diff without being a real gap.
// Read-only; safe to call anytime.
export async function getTreasuryReconciliation() {
  const { rows: t } = await sql`SELECT to_regclass('cash_boxes') AS t`;
  if (!t[0]?.t) return { available: false };
  const r2 = (n) => Math.round((parseFloat(n) || 0) * 100) / 100;

  const { rows: bx } = await sql`SELECT COALESCE(SUM(balance), 0) AS s FROM cash_boxes`;
  const boxesTotal = r2(bx[0]?.s);

  const { rows: mv } = await sql`
    SELECT
      COALESCE(SUM(signed_amount) FILTER (WHERE kind = 'opening'), 0) AS opening,
      COALESCE(SUM(signed_amount) FILTER (WHERE kind <> 'opening'), 0) AS activity,
      COALESCE(SUM(signed_amount), 0) AS total
    FROM cash_movements`;
  const openingTotal = r2(mv[0]?.opening);
  const movementsActivity = r2(mv[0]?.activity);
  const movementsTotal = r2(mv[0]?.total);

  const { rows: cut } = await sql`SELECT MIN(date)::text AS d FROM cash_movements WHERE kind = 'opening'`;
  const cutoff = cut[0]?.d || null;
  const integrityDiff = r2(boxesTotal - movementsTotal);

  let coverage = null;
  if (cutoff) {
    const { rows: c } = await sql`SELECT COALESCE(SUM(amount), 0) AS s FROM payments WHERE date >= ${cutoff}`;
    const collections = r2(c[0]?.s);
    const { rows: sp } = await sql`SELECT COALESCE(SUM(amount), 0) AS s FROM supplier_payments WHERE date >= ${cutoff}`;
    const supplierPaid = r2(sp[0]?.s);
    const { rows: ex } = await sql`SELECT COALESCE(SUM(amount), 0) AS s FROM expenses WHERE date >= ${cutoff}`;
    const expenses = r2(ex[0]?.s);
    const { rows: st } = await sql`SELECT COALESCE(SUM(amount), 0) AS s FROM settlements
      WHERE type IN ('seller_payout', 'driver_payout') AND amount > 0 AND date >= ${cutoff}`;
    const payouts = r2(st[0]?.s);
    const { rows: pd } = await sql`SELECT COALESCE(SUM(amount), 0) AS s FROM profit_distributions WHERE created_at::date >= ${cutoff}::date`;
    const distributions = r2(pd[0]?.s);
    let capitalNet = 0;
    try {
      const { rows: cap } = await sql`SELECT COALESCE(SUM(CASE WHEN kind = 'injection' THEN amount ELSE -amount END), 0) AS s
        FROM capital_operations WHERE status = 'approved' AND resolved_at::date >= ${cutoff}::date`;
      capitalNet = r2(cap[0]?.s);
    } catch { capitalNet = 0; } // table may be absent pre-migration
    const coreNet = r2(collections - supplierPaid - expenses - payouts - distributions + capitalNet);
    coverage = {
      collections, supplierPaid, expenses, payouts, distributions, capitalNet,
      coreNet, movementsActivity, diff: r2(movementsActivity - coreNet),
    };
  }

  return { available: true, cutoff, boxesTotal, openingTotal, movementsActivity, movementsTotal, integrityDiff, coverage };
}

// Resolve a box id from {boxId | owner | general}. Returns null if none.
async function resolveBoxId(client, { boxId, owner, general }) {
  if (boxId) return boxId;
  if (owner) {
    const { rows } = await client.sql`SELECT id FROM cash_boxes WHERE owner_username = ${owner}`;
...
// ── Treasury (Phase 1) — funding / handover dual-confirm state machine ─────
// A transfer between two boxes is a two-step handshake: initiate (creates a
// 'pending' cash_handovers row, NO money moves) → the other party confirms
// (balances move atomically, status → 'confirmed') OR reject (status →
// 'rejected', nothing moves). All gated by TREASURY_ENABLED. The DB layer is
// pure mechanics — WHO may initiate/confirm (counterparty, admins for the
// general box, self-transfers) is enforced at the API layer.
// The `*InTx` cores take a client so tests can inject a mock (no real DB).
export async function initiateHandoverInTx(client, data) {
  if (!(await isTreasuryEnabled(client))) throw new Error('نظام الصناديق غير مفعّل');
  const amount = Math.round((parseFloat(data.amount) || 0) * 100) / 100;
  if (amount <= 0) throw new Error('المبلغ يجب أن يكون أكبر من صفر');
  if (!data.initiatedBy) throw new Error('المُبادِر مطلوب');
  const kind = data.kind === 'funding' ? 'funding' : 'handover';
  const fromId = await resolveBoxId(client, { boxId: data.fromBoxId, owner: data.fromOwner, general: data.fromGeneral });
  const toId = await resolveBoxId(client, { boxId: data.toBoxId, owner: data.toOwner, general: data.toGeneral });
  if (!fromId || !toId) throw new Error('الصندوق غير موجود');
  if (fromId === toId) throw new Error('لا يمكن التحويل إلى نفس الصندوق');
  const { rows } = await client.sql`
    INSERT INTO cash_handovers (kind, from_box_id, to_box_id, amount, method, status, initiated_by, notes)
    VALUES (${kind}, ${fromId}, ${toId}, ${amount}, ${data.method || 'كاش'}, 'pending', ${data.initiatedBy}, ${data.notes || ''})
    RETURNING id
  `;
  return { id: rows[0].id, fromBoxId: fromId, toBoxId: toId };
}
export async function initiateHandover(data) {
  return withTx((client) => initiateHandoverInTx(client, data));
}

export async function confirmHandoverInTx(client, handoverId, confirmedBy) {
  if (!(await isTreasuryEnabled(client))) throw new Error('نظام الصناديق غير مفعّل');
  const { rows } = await client.sql`SELECT * FROM cash_handovers WHERE id = ${handoverId} FOR UPDATE`;
  if (!rows.length) throw new Error('طلب التسليم غير موجود');
  const h = rows[0];
  if (h.status !== 'pending') throw new Error('لا يمكن تأكيد طلب غير معلّق');
  const amount = parseFloat(h.amount) || 0;
  // Money moves ONLY here, atomically: from_box −amount, to_box +amount.
  await client.sql`
    INSERT INTO cash_movements (date, box_id, counterparty_box_id, signed_amount, method, kind, source_table, source_id, created_by, notes)
    VALUES (CURRENT_DATE, ${h.from_box_id}, ${h.to_box_id}, ${-amount}, ${h.method}, ${h.kind}, 'cash_handovers', ${handoverId}, ${confirmedBy || ''}, '')
  `;
  await client.sql`UPDATE cash_boxes SET balance = balance - ${amount} WHERE id = ${h.from_box_id}`;
  await client.sql`
    INSERT INTO cash_movements (date, box_id, counterparty_box_id, signed_amount, method, kind, source_table, source_id, created_by, notes)
    VALUES (CURRENT_DATE, ${h.to_box_id}, ${h.from_box_id}, ${amount}, ${h.method}, ${h.kind}, 'cash_handovers', ${handoverId}, ${confirmedBy || ''}, '')
  `;
  await client.sql`UPDATE cash_boxes SET balance = balance + ${amount} WHERE id = ${h.to_box_id}`;
  await client.sql`UPDATE cash_handovers SET status = 'confirmed', confirmed_by = ${confirmedBy || ''}, resolved_at = NOW() WHERE id = ${handoverId}`;
  return { success: true };
}
export async function confirmHandover(handoverId, confirmedBy) {
  return withTx((client) => confirmHandoverInTx(client, handoverId, confirmedBy));
}

export async function rejectHandoverInTx(client, handoverId, rejectedBy) {
  const { rows } = await client.sql`SELECT status FROM cash_handovers WHERE id = ${handoverId} FOR UPDATE`;
  if (!rows.length) throw new Error('طلب التسليم غير موجود');
  if (rows[0].status !== 'pending') throw new Error('لا يمكن رفض طلب غير معلّق');
  await client.sql`UPDATE cash_handovers SET status = 'rejected', confirmed_by = ${rejectedBy || ''}, resolved_at = NOW() WHERE id = ${handoverId}`;
  return { success: true };
}
export async function rejectHandover(handoverId, rejectedBy) {
  return withTx((client) => rejectHandoverInTx(client, handoverId, rejectedBy));
}

// ── Treasury — capital operations (money IN/OUT of the project) ────────────
// Require ALL active admins to approve before the general box moves. With a
// single admin, the initiator's own approval suffices → applied immediately.
// Applies the move once, atomically, when the last required approval lands.
async function applyCapitalOpInTx(client, op, byUser) {
  const { rows: g } = await client.sql`SELECT id FROM cash_boxes WHERE type = 'main' LIMIT 1`;
  const generalId = g[0]?.id;
  if (!generalId) throw new Error('الصندوق العام غير موجود');
  const amount = parseFloat(op.amount) || 0;
  const delta = op.kind === 'injection' ? amount : -amount;
  await client.sql`
    INSERT INTO cash_movements (date, box_id, signed_amount, method, kind, source_table, source_id, created_by, notes)
    VALUES (CURRENT_DATE, ${generalId}, ${delta}, ${op.method}, ${op.kind === 'injection' ? 'capital_injection' : 'capital_withdrawal'}, 'capital_operations', ${op.id}, ${byUser || ''}, ${op.notes || ''})
  `;
  await client.sql`UPDATE cash_boxes SET balance = balance + ${delta} WHERE id = ${generalId}`;
  await client.sql`UPDATE capital_operations SET status = 'approved', resolved_at = NOW() WHERE id = ${op.id}`;
}

// If every active admin has approved op #opId (still pending), apply it.
async function maybeApplyCapitalOp(client, opId, byUser) {
  const { rows: op } = await client.sql`SELECT * FROM capital_operations WHERE id = ${opId} FOR UPDATE`;
  if (!op[0] || op[0].status !== 'pending') return;
  const { rows: ac } = await client.sql`SELECT COUNT(*)::int AS n FROM users WHERE role = 'admin' AND active = true`;
  const adminCount = ac[0]?.n || 0;
  const { rows: ap } = await client.sql`
    SELECT COUNT(DISTINCT a.username)::int AS n
    FROM capital_op_approvals a JOIN users u ON u.username = a.username
    WHERE a.operation_id = ${opId} AND u.role = 'admin' AND u.active = true`;
  const approved = ap[0]?.n || 0;
  if (adminCount > 0 && approved >= adminCount) {
    await applyCapitalOpInTx(client, op[0], byUser);
  }
}

export async function initiateCapitalOpInTx(client, data) {
  if (!(await isTreasuryEnabled(client))) throw new Error('نظام الصناديق غير مفعّل');
  const kind = data.kind === 'withdrawal' ? 'withdrawal' : 'injection';
  const amount = Math.round((parseFloat(data.amount) || 0) * 100) / 100;
  if (amount <= 0) throw new Error('المبلغ يجب أن يكون أكبر من صفر');
  if (!data.initiatedBy) throw new Error('المُبادِر مطلوب');
  const { rows } = await client.sql`
    INSERT INTO capital_operations (kind, amount, method, status, initiated_by, notes)
    VALUES (${kind}, ${amount}, ${data.method === 'بنك' ? 'بنك' : 'كاش'}, 'pending', ${data.initiatedBy}, ${data.notes || ''})
    RETURNING id`;
  const opId = rows[0].id;
  await client.sql`INSERT INTO capital_op_approvals (operation_id, username) VALUES (${opId}, ${data.initiatedBy}) ON CONFLICT DO NOTHING`;
  await maybeApplyCapitalOp(client, opId, data.initiatedBy); // single-admin → applies now
  const { rows: st } = await client.sql`SELECT status FROM capital_operations WHERE id = ${opId}`;
  return { id: opId, status: st[0]?.status };
}
export async function initiateCapitalOp(data) {
  return withTx((client) => initiateCapitalOpInTx(client, data));
}

export async function approveCapitalOpInTx(client, opId, adminUsername) {
  if (!(await isTreasuryEnabled(client))) throw new Error('نظام الصناديق غير مفعّل');
  const { rows: op } = await client.sql`SELECT status FROM capital_operations WHERE id = ${opId} FOR UPDATE`;
  if (!op[0]) throw new Error('العملية غير موجودة');
  if (op[0].status !== 'pending') throw new Error('العملية غير معلّقة');
  const { rows: adm } = await client.sql`SELECT 1 FROM users WHERE username = ${adminUsername} AND role = 'admin' AND active = true`;
  if (!adm.length) throw new Error('فقط المدراء العامون يمكنهم الموافقة');
  await client.sql`INSERT INTO capital_op_approvals (operation_id, username) VALUES (${opId}, ${adminUsername}) ON CONFLICT DO NOTHING`;
  await maybeApplyCapitalOp(client, opId, adminUsername);
  const { rows: st } = await client.sql`SELECT status FROM capital_operations WHERE id = ${opId}`;
  return { status: st[0]?.status };
}
export async function approveCapitalOp(opId, adminUsername) {
  return withTx((client) => approveCapitalOpInTx(client, opId, adminUsername));
}

export async function rejectCapitalOpInTx(client, opId, adminUsername) {
  if (!(await isTreasuryEnabled(client))) throw new Error('نظام الصناديق غير مفعّل');
  const { rows: op } = await client.sql`SELECT status FROM capital_operations WHERE id = ${opId} FOR UPDATE`;
  if (!op[0]) throw new Error('العملية غير موجودة');
  if (op[0].status !== 'pending') throw new Error('العملية غير معلّقة');
  const { rows: adm } = await client.sql`SELECT 1 FROM users WHERE username = ${adminUsername} AND role = 'admin' AND active = true`;
  if (!adm.length) throw new Error('فقط المدراء العامون يمكنهم الرفض');
  await client.sql`UPDATE capital_operations SET status = 'rejected', resolved_at = NOW() WHERE id = ${opId}`;
  return { status: 'rejected' };
}
export async function rejectCapitalOp(opId, adminUsername) {
  return withTx((client) => rejectCapitalOpInTx(client, opId, adminUsername));
}

// Pending capital ops + how many of the required admin approvals are in.
export async function getPendingCapitalOps() {
  const { rows: t } = await sql`SELECT to_regclass('capital_operations') AS t`;
  if (!t[0]?.t) return [];
  const { rows } = await sql`
    SELECT o.id, o.kind, o.amount, o.method, o.status, o.initiated_by, o.created_at,
      (SELECT COUNT(DISTINCT a.username)::int FROM capital_op_approvals a
         JOIN users u ON u.username = a.username
         WHERE a.operation_id = o.id AND u.role = 'admin' AND u.active = true) AS approvals,
      (SELECT COUNT(*)::int FROM users WHERE role = 'admin' AND active = true) AS admin_count
    FROM capital_operations o WHERE o.status = 'pending' ORDER BY o.created_at DESC`;
  return rows;
}

// Full capital-operations LOG (read-only) — every injection / withdrawal in any
// status (approved / pending / rejected), newest first, with the initiator's
// name. Powers the "سجلّ رأس المال" history on the treasury page so the admin
// can see exactly who put capital in or took it out, and when.
export async function getCapitalHistory(limit = 200) {
  const { rows: t } = await sql`SELECT to_regclass('capital_operations') AS t`;
  if (!t[0]?.t) return [];
  const { rows } = await sql`
    SELECT o.id, o.kind, o.amount, o.method, o.status, o.initiated_by, o.notes,
           o.created_at, o.resolved_at, u.name AS initiator_name
    FROM capital_operations o
    LEFT JOIN users u ON u.username = o.initiated_by
    ORDER BY o.created_at DESC
    LIMIT ${limit}`;
  return rows;
}

// Authority: can `user` (role, username) MANAGE (move money in/out of) this box?
// Derived from the locked model: admin → own + the general box; manager → own +
// any driver's box; driver → own. (Viewing is broader; managing is this.) Pure
// function (box = {type, owner_username, owner_role}) → exported for unit tests.
export function managesBox(role, username, box) {
  if (!box) return false;
  if (box.type === 'main') return role === 'admin';
  if (box.owner_username && box.owner_username === username) return true;
  if (role === 'manager' && box.owner_role === 'driver') return true;
  return false;
}

// A transfer between two boxes is valid unless both are custody boxes of the
// SAME role (no driver↔driver / manager↔manager / admin↔admin). general↔custody
// and different-level custody pairs are allowed.
export function validTransferPair(fromBox, toBox) {
  if (!fromBox || !toBox || fromBox.id === toBox.id) return false;
  if (fromBox.type === 'custody' && toBox.type === 'custody' &&
      fromBox.owner_role && fromBox.owner_role === toBox.owner_role) return false;
  return true;
}

// Resolve a box (with owner role + balance) from {boxId | owner | general}.
export async function getBoxRef({ boxId, owner, general } = {}) {
  const { rows: t } = await sql`SELECT to_regclass('cash_boxes') AS t`;
  if (!t[0]?.t) return null;
  if (boxId) {
    const { rows } = await sql`SELECT b.id, b.type, b.owner_username, b.balance, u.role AS owner_role
      FROM cash_boxes b LEFT JOIN users u ON u.username = b.owner_username WHERE b.id = ${boxId}`;
    return rows[0] || null;
  }
  if (general) {
    const { rows } = await sql`SELECT b.id, b.type, b.owner_username, b.balance, u.role AS owner_role
      FROM cash_boxes b LEFT JOIN users u ON u.username = b.owner_username WHERE b.type = 'main' LIMIT 1`;
    return rows[0] || null;
  }
  if (owner) {
    const { rows } = await sql`SELECT b.id, b.type, b.owner_username, b.balance, u.role AS owner_role
      FROM cash_boxes b LEFT JOIN users u ON u.username = b.owner_username WHERE b.owner_username = ${owner}`;
    return rows[0] || null;
  }
  return null;
}

// Get a single pending/any handover with both boxes' display + role info.
export async function getHandoverById(id) {
  const { rows: t } = await sql`SELECT to_regclass('cash_handovers') AS t`;
  if (!t[0]?.t) return null;
  const { rows } = await sql`
    SELECT h.*,
      bf.type AS from_type, bf.owner_username AS from_owner, uf.role AS from_role,
      bt.type AS to_type, bt.owner_username AS to_owner, ut.role AS to_role
    FROM cash_handovers h
    LEFT JOIN cash_boxes bf ON bf.id = h.from_box_id
    LEFT JOIN cash_boxes bt ON bt.id = h.to_box_id
    LEFT JOIN users uf ON uf.username = bf.owner_username
    LEFT JOIN users ut ON ut.username = bt.owner_username
    WHERE h.id = ${id}`;
  return rows[0] || null;
}

// Pending handovers the user is allowed to act on (manages either side); admin sees all.
export async function getPendingHandoversForUser(role, username) {
  const { rows: t } = await sql`SELECT to_regclass('cash_handovers') AS t`;
  if (!t[0]?.t) return [];
  const { rows } = await sql`
    SELECT h.id, h.kind, h.amount, h.method, h.status, h.initiated_by, h.created_at,
      bf.type AS from_type, bf.owner_username AS from_owner, uf.role AS from_role, uf.name AS from_name,
      bt.type AS to_type, bt.owner_username AS to_owner, ut.role AS to_role, ut.name AS to_name
    FROM cash_handovers h
    LEFT JOIN cash_boxes bf ON bf.id = h.from_box_id
    LEFT JOIN cash_boxes bt ON bt.id = h.to_box_id
    LEFT JOIN users uf ON uf.username = bf.owner_username
  ...
  return rows.filter((h) =>
    managesBox(role, username, { type: h.from_type, owner_username: h.from_owner, owner_role: h.from_role }) ||
    managesBox(role, username, { type: h.to_type, owner_username: h.to_owner, owner_role: h.to_role })
  );
}

// Read-side: the boxes a viewer may SEE (RBAC).
//   admin   → all boxes (incl. the general box)
//   manager → own box + all drivers' boxes
//   driver  → own box only
//   seller  → none
// A custody box of a NON-cash role (e.g. a driver who became a seller) is
// hidden once it is empty — but a box that still HOLDS money is always shown,
// so orphaned cash can never be hidden from the admin (money-safety).
// Guarded for the pre-migration window (returns [] if the table is absent).
export async function getCashBoxesForViewer(role, username) {
  const { rows: t } = await sql`SELECT to_regclass('cash_boxes') AS t`;
  if (!t[0]?.t) return [];
  if (role === 'admin') {
    const { rows } = await sql`
      SELECT b.id, b.owner_username, b.type, b.balance, b.active,
             u.name AS owner_name, u.role AS owner_role,
             (SELECT m.signed_amount FROM cash_movements m
                WHERE m.box_id = b.id AND m.kind = 'opening' LIMIT 1) AS opening
      FROM cash_boxes b LEFT JOIN users u ON u.username = b.owner_username
      WHERE b.type = 'main'
         OR (COALESCE(b.active, true) AND u.role IN ('admin','manager','driver'))
         OR ABS(COALESCE(b.balance, 0)) > 0.005
      ORDER BY (b.type = 'main') DESC, u.role, u.name`;
    return rows;
  }
  if (role === 'manager') {
    const { rows } = await sql`
      SELECT b.id, b.owner_username, b.type, b.balance, b.active,
             u.name AS owner_name, u.role AS owner_role
      FROM cash_boxes b LEFT JOIN users u ON u.username = b.owner_username
      WHERE (b.owner_username = ${username} OR u.role = 'driver') AND COALESCE(b.active, true)
      ORDER BY (b.owner_username = ${username}) DESC, u.name`;
    return rows;
  }
  if (role === 'driver') {
    const { rows } = await sql`
      SELECT b.id, b.owner_username, b.type, b.balance, b.active,
             u.name AS owner_name, u.role AS owner_role
      FROM cash_boxes b LEFT JOIN users u ON u.username = b.owner_username
      WHERE b.owner_username = ${username} AND COALESCE(b.active, true)`;
    return rows;
  }
  return [];
}

// Per-box money-movement ledger (read-only) — the full audit trail of every
// in/out for one cash box, newest first, with a RUNNING BALANCE per row. Used by
// the "حركات الأموال" tabs on the treasury page so admins can follow every
// person's money moves and each holder can review their own box. Pure SELECT —
// no money logic, no writes. The running balance is summed in chronological
// (date,id) order via a window so the newest row's balance equals the box's
// current balance, even though rows are returned newest-first for display.
// Caller MUST authorise box visibility first (see the movements route).
export async function getBoxMovements(boxId) {
  const { rows: t } = await sql`SELECT to_regclass('cash_movements') AS t`;
  if (!t[0]?.t) return [];
  const { rows } = await sql`
    SELECT m.id,
           m.date::text AS date,
           ...
           m.signed_amount,
           m.method,
           m.counterparty_box_id,
           cb.type AS counterparty_type,
           cu.name AS counterparty_name,
           cb.owner_username AS counterparty_username,
           m.created_by,
           m.notes,
           m.source_table,
           m.source_id,
    ...
    LEFT JOIN cash_boxes cb ON cb.id = m.counterparty_box_id
    LEFT JOIN users cu ON cu.username = cb.owner_username
...
export async function applyCollectionInTx(client, saleId, amountTTC, paymentMethod, collectedBy) {
  ...
  // Treasury shadow write (no-op unless TREASURY_ENABLED): the collected
  // amount enters the collector's custody box. method is just a tag.
  await emitCashMovement(client, {
    ownerUsername: collectedBy,
    signedAmount: amount,
    method: paymentMethod === 'بنك' ? 'بنك' : 'كاش',
    kind: 'collection',
    sourceTable: 'payments',
    sourceId: paymentRows[0].id,
...
/**
 * Bank-receipt confirmation. For a بنك sale, an admin/manager confirms that the
 * funds arrived in the bank account BEFORE delivery. This records the bank
 * collection payment (attributed to the confirmer — so the treasury layer can
 * later credit their custody box), marks the sale paid, and stamps
 * bank_received_by/at. The sale stays 'محجوز'; the delivery-confirm gate in
 * updateDelivery refuses a بنك sale until bank_received_by is set.
 *
 * @param {number} saleId
 * @param {string} confirmedBy  username of the admin/manager confirming receipt
 * @returns {Promise<{success:true, amount:number}>}
 */
export async function confirmBankReceipt(saleId, confirmedBy) {
  await ensureBankReceivedColumn(); // self-heal the bank_received_* columns (migration-timing safety)
  return withTx(async (client) => {
    const { rows } = await client.sql`SELECT * FROM sales WHERE id = ${saleId} FOR UPDATE`;
    if (!rows.length) throw new Error('الطلب غير موجود');
    const sale = rows[0];
    if (sale.status === 'ملغي') throw new Error('لا يمكن تأكيد طلب ملغي');
    if (sale.payment_type !== 'بنك') throw new Error('هذا الطلب ليس بنكياً');
    if (sale.bank_received_by) throw new Error('تم تأكيد استلام المبلغ البنكي مسبقاً');

    const total = parseFloat(sale.total) || 0;
    const { rows: vatRows } = await client.sql`SELECT value FROM settings WHERE key = 'vat_rate'`;
    const vatRate = parseFloat(vatRows[0]?.value) || 20;
    const tva = Math.round((total * vatRate / (100 + vatRate)) * 100) / 100;

    const { rows: payRows } = await client.sql`
      INSERT INTO payments (
        date, client_name, amount, sale_id,
        type, payment_method, tva_amount, created_by, notes
      ) VALUES (
        CURRENT_DATE, ${sale.client_name}, ${total}, ${saleId},
        'collection', 'بنك', ${tva}, ${confirmedBy}, 'تأكيد استلام بنكي'
      )
      RETURNING id
    `;
    await client.sql`
      UPDATE sales
      SET paid_amount = ${total}, remaining = 0, payment_status = 'paid',
          bank_received_by = ${confirmedBy}, bank_received_at = NOW()
      WHERE id = ${saleId}
    `;
    // Treasury shadow (no-op unless TREASURY_ENABLED): the confirmed bank funds
    // enter the confirming admin/manager's box.
    await emitCashMovement(client, {
      ownerUsername: confirmedBy,
      signedAmount: total,
      method: 'بنك',
      kind: 'collection',
      sourceTable: 'payments',
      sourceId: payRows[0].id,
 ...
 * @returns {Promise<Array<object>>}
 */
// HOTFIX: getDeliveries reads sales.bank_received_by. If initDatabase has not
// been re-run after the bank-workflow deploy, that column is absent and the
// JOIN query 500s → the deliveries page shows EMPTY (no pending/in-transit
// rows, no history). Self-heal the column once per process (idempotent ALTER,
// a no-op once present) so the page never depends on migration timing again.
let bankReceivedColEnsured = false;
async function ensureBankReceivedColumn() {
  if (bankReceivedColEnsured) return;
  await sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS bank_received_by TEXT`.catch(() => {});
  await sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS bank_received_at TIMESTAMPTZ`.catch(() => {});
  bankReceivedColEnsured = true;
}

export async function getDeliveries(status, assignedDriver, createdBy) {
  await ensureBankReceivedColumn();
  ...
  if (status && assignedDriver) {
    const { rows } = await sql`
      SELECT d.*, s.down_payment_expected, s.payment_type AS sale_payment_type, s.bank_received_by
      FROM deliveries d
...
export async function updateDelivery(data) {
  return withTx(async (client) => {
    ...
    if (data.status === 'تم التوصيل') {
      ...
      // Bank-receipt gate: a بنك sale cannot be delivered until an admin/manager
      // confirmed the funds arrived (confirmBankReceipt). The confirmer already
      // recorded the payment + paid_amount; delivery only promotes the status.
      if (sale.payment_type === 'بنك' && !sale.bank_received_by) {
        throw new Error('لا يمكن التسليم قبل تأكيد استلام المبلغ البنكي من مدير عام أو مشرف');
      }
      ...
      if (sale.payment_type === 'بنك') {
        // Bank funds were already recorded at confirmBankReceipt (paid_amount,
        // payment_status='paid', and the بنك collection row attributed to the
        // confirming admin/manager). Delivery only promotes status to 'مؤكد' —
        // it never re-records the payment nor touches paid_amount.
        await client.sql`UPDATE sales SET status = 'مؤكد' WHERE id = ${saleId}`;
      } else {
        // FEAT-04: كاش / آجل confirm using down_payment_expected. For كاش
        // dpe = total; for آجل dpe may be 0 (pure credit) or a forced door
        // down payment. paid_amount is set to what the driver collected.
        ...
        // Cash collected at the door → a collection payment row (always كاش
        // here; bank never reaches this branch). cancelSale() refunds it.
        if (dpe > 0.005) {
          ...
          const { rows: colRows } = await client.sql`
            INSERT INTO payments (
              date, client_name, amount, sale_id,
              type, payment_method, tva_amount, created_by, notes
            ) VALUES (
              CURRENT_DATE, ${sale.client_name}, ${dpe}, ${saleId},
              'collection', 'كاش', ${tvaAmount}, ${driverForPayment}, ''
            )
            RETURNING id
          `;
          // Treasury shadow (no-op unless TREASURY_ENABLED): cash collected at
          // the door enters the delivering driver's box.
          await emitCashMovement(client, {
            ownerUsername: driverForPayment,
            signedAmount: dpe,
            method: 'كاش',
            kind: 'collection',
            sourceTable: 'payments',
            sourceId: colRows[0].id,
            createdBy: driverForPayment,
          });
        }
      }
 ...
 */
export async function getSummaryData(from, to, includePrev = true) {
  // `includePrev` (default true): also compute the previous comparable period's
  // headline numbers for the dashboard's ↑↓ delta. The recursive call passes
  // false to avoid infinite recursion. Purely additive — never affects the
  // current-period figures below.
  //
  ...
  // Dashboard revamp — total supplier payables ("what we owe") across ALL
  // suppliers, not just the top-10 slice. Mirrors totalDebt ("what we're owed").
  const supplierPayablesTotal = Object.values(supplierMap)
    .reduce((s, r) => s + (r.totalRemaining || 0), 0);
  ...
  // Dashboard revamp — full "awaiting delivery" queue (pending + in-transit)
  // with its goods value, for the admin/manager operational card. recentDeliveries
  // (top 5) is kept below for backward compat.
  const awaitingDeliveryList = [...pendingDeliveries, ...inTransitDeliveries];
  const awaitingDeliveryValue = awaitingDeliveryList
  ...
  // ─── Dashboard revamp — additive operational metrics ────────────────
  // All guarded: a failure here (e.g. an older DB missing a column) must
  // NEVER break the core P&L summary above. Each defaults to a safe zero.
  const todayStr = new Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/Amsterdam' }).format(new Date());
  let todayRevenue = 0, todayCollections = 0, todayExpenses = 0;
  let bankPendingCount = 0, bankPendingValue = 0, bankPending = [];
  try {
    const [todaySalesR, todayCollR, todayExpR, bankPendR] = await Promise.all([
      sql`SELECT COALESCE(SUM(total),0) AS t FROM sales WHERE date = ${todayStr} AND status = 'مؤكد'`,
      sql`SELECT COALESCE(SUM(amount),0) AS t FROM payments WHERE date = ${todayStr} AND type IN ('collection','refund')`,
      sql`SELECT COALESCE(SUM(amount),0) AS t FROM expenses WHERE date = ${todayStr}`,
      sql`SELECT id, date, client_name, item, total FROM sales
            WHERE status = 'محجوز' AND payment_type = 'بنك'
              AND (bank_received_by IS NULL OR bank_received_by = '')
            ORDER BY date ASC`,
    ]);
    todayRevenue = parseFloat(todaySalesR.rows[0]?.t) || 0;
    todayCollections = parseFloat(todayCollR.rows[0]?.t) || 0;
    todayExpenses = parseFloat(todayExpR.rows[0]?.t) || 0;
    bankPending = bankPendR.rows;
    bankPendingCount = bankPending.length;
    bankPendingValue = bankPending.reduce((s, r) => s + (parseFloat(r.total) || 0), 0);
  } catch (e) {
    console.warn('[getSummaryData] additive metrics skipped:', e.message);
  }

  // Previous comparable period (same length, immediately before [from,to]) for
  // the ↑↓ delta on the headline KPIs. Only when an explicit range is set;
  // the recursive call passes includePrev=false so it never recurses again.
  let prevTotalRevenue = null, prevNetProfitCashBasis = null;
  if (includePrev && from && to) {
    try {
      ...
      const prev = await getSummaryData(iso(prevFrom), iso(prevTo), false);
      prevTotalRevenue = prev.totalRevenue;
      prevNetProfitCashBasis = prev.netProfitCashBasis;
    } catch (e) {
      console.warn('[getSummaryData] previous-period delta skipped:', e.message);
    }
  }
  ...
  return {
    ...
    // Dashboard revamp — full awaiting-delivery queue + its goods value,
    // and total supplier payables. All additive.
    awaitingDelivery: awaitingDeliveryList.slice(0, 50),
    awaitingDeliveryCount: awaitingDeliveryList.length,
    awaitingDeliveryValue,
    supplierPayablesTotal,
    // Dashboard revamp — today's headline figures + bank-receipt queue +
    // previous-period deltas (see the additive-metrics block above).
    todayRevenue,
    todayCollections,
    todayExpenses,
    bankPendingCount,
...
export async function addUser(data) {
  ...
  const { rows } = await sql`
    ...
    VALUES (${data.username}, ${hash}, ${data.name}, ${role}, true)
    RETURNING id
  `;
  // Treasury (Phase 1): auto-create a custody box for cash-handling roles.
  // Best-effort — must NEVER break user creation (the user row is already
  // committed; the global sql is not transactional). If the table doesn't
  // exist yet (pre-migration) initDatabase will backfill the box on next run.
  await ensureCashBox(data.username, role);
  return rows[0].id;
}

// Idempotent, non-fatal custody-box creation for a cash-handling user.
async function ensureCashBox(username, role) {
  if (!username || !['admin', 'manager', 'driver'].includes(role)) return;
  try {
    await sql`
      INSERT INTO cash_boxes (owner_username, type)
      SELECT ${username}, 'custody'
      WHERE NOT EXISTS (SELECT 1 FROM cash_boxes WHERE owner_username = ${username})
    `;
    // Reactivate a box that was deactivated when the user was a non-cash role
    // (e.g. seller → driver again). History is preserved; the box reappears.
    await sql`UPDATE cash_boxes SET active = true WHERE owner_username = ${username} AND active = false`;
  } catch { /* inert until treasury is enabled; backfilled by initDatabase */ }
}
...
export async function updateUser(data) {
  // Treasury safety: block a ROLE change while the user's custody box still
  // holds money — the cash must be settled (handed over) first so it is never
  // orphaned by the role change. The box belongs to the person, so on a normal
  // role change the balance simply stays in the same box. Enforced ONLY when
  // the treasury is live (TREASURY_ENABLED), so the current users flow is
  // untouched until then.
  if (data.role && await isTreasuryEnabled()) {
    const { rows: cur } = await sql`SELECT username, role FROM users WHERE id = ${data.id}`;
    const u0 = cur[0];
    if (u0 && data.role !== u0.role) {
      const { rows: b } = await sql`SELECT balance FROM cash_boxes WHERE owner_username = ${u0.username}`;
      const bal = parseFloat(b[0]?.balance) || 0;
      if (Math.abs(bal) > 0.005) {
        throw new Error(`لا يمكن تغيير الدور: يوجد رصيد (${bal.toFixed(2)}) في صندوق العهدة — يجب تسليمه (تصفيره) أولاً ثم تغيير الدور`);
      }
    }
  }
  if (data.password) {
  ...
  // Treasury (Phase 1): keep the custody box in sync with the (new) role.
  //  - cash role (admin/manager/driver) → ensure an ACTIVE box exists.
  //  - non-cash role (e.g. seller)      → deactivate the box IF it is empty, so
  //    it drops out of the money views (a seller holds no custody). Never
  //    delete: any movement history is kept. The role-change guard above
  //    already blocks the change when a balance remains, and we re-check
  //    balance ≈ 0 here as a belt-and-suspenders before deactivating.
  // Best-effort; never breaks the edit (inert/guarded until treasury exists).
  try {
    const { rows } = await sql`SELECT username FROM users WHERE id = ${data.id}`;
    const uname = rows[0]?.username;
    if (uname) {
      if (['admin', 'manager', 'driver'].includes(data.role)) {
        await ensureCashBox(uname, data.role);
      } else {
        await sql`UPDATE cash_boxes SET active = false
                  WHERE owner_username = ${uname} AND type = 'custody'
                    AND ABS(COALESCE(balance, 0)) <= 0.005`;
      }
    }
  } catch { /* inert; backfilled by initDatabase */ }
}
...
export async function deleteUser(id) {
  // The user's custody box references users(username) ON DELETE RESTRICT, so a
  // seeded box would otherwise block deletion. Handle it with money-safety:
  //  - non-zero balance  → block (settle first; never orphan cash)
  //  - empty + no history → remove the box, then the user
  //  - has movement history → block (deactivate instead of delete)
  const { rows: u } = await sql`SELECT username FROM users WHERE id = ${id}`;
  const username = u[0]?.username;
  if (username) {
    const { rows: tbl } = await sql`SELECT to_regclass('cash_boxes') AS t`;
    if (tbl[0]?.t) {
      const { rows: b } = await sql`SELECT id, balance FROM cash_boxes WHERE owner_username = ${username}`;
      if (b[0]) {
        const bal = parseFloat(b[0].balance) || 0;
        if (Math.abs(bal) > 0.005) {
          throw new Error('لا يمكن حذف المستخدم: يوجد رصيد في صندوق عهدته — يجب تسليمه أولاً');
        }
        try {
          await sql`DELETE FROM cash_boxes WHERE id = ${b[0].id}`;
        } catch {
          throw new Error('لا يمكن حذف المستخدم: لديه سجلّ حركات مالية — يُفضّل تعطيله بدل حذفه');
        }
      }
    }
  }
...
const SETTINGS_WHITELIST = new Set([
  ...
  'vat_rate', 'invoice_currency', 'treasury_enabled',
...
// Per-category bonus rate overrides — same shape/contract as the per-user
// functions above, but keyed on the product's category. A missing row (or a
// NULL column) means "fall back to the global settings value" — so existing
// behavior is unchanged until the admin sets a category rate.
export async function getCategoryBonusRates() {
  // Guard: table may not exist before /api/init → return no overrides (global rates apply).
  const { rows: t } = await sql`SELECT to_regclass('category_bonus_rates') AS t`;
  if (!t[0]?.t) return [];
  const { rows } = await sql`SELECT * FROM category_bonus_rates ORDER BY category`;
  return rows;
}

/**
 * Upsert a per-category bonus rate. Only provided (non-null) fields are
 * written; null/undefined fields keep the existing value (blank = "no
 * change"), matching setUserBonusRate's semantics. Delete the row to revert
 * the category to the global defaults.
 * @param {{category:string, seller_fixed?:number, seller_percentage?:number, driver_fixed?:number, updatedBy:string}} data
 */
export async function setCategoryBonusRate(data) {
  const sf = data.seller_fixed != null ? parseFloat(data.seller_fixed) : null;
  const sp = data.seller_percentage != null ? parseFloat(data.seller_percentage) : null;
  const df = data.driver_fixed != null ? parseFloat(data.driver_fixed) : null;
  await sql`
    INSERT INTO category_bonus_rates (category, seller_fixed, seller_percentage, driver_fixed, updated_by, updated_at)
    VALUES (${data.category}, ${sf}, ${sp}, ${df}, ${data.updatedBy}, NOW())
    ON CONFLICT (category) DO UPDATE SET
      seller_fixed = COALESCE(${sf}, category_bonus_rates.seller_fixed),
      seller_percentage = COALESCE(${sp}, category_bonus_rates.seller_percentage),
      driver_fixed = COALESCE(${df}, category_bonus_rates.driver_fixed),
      updated_by = ${data.updatedBy},
      updated_at = NOW()
  `;
}

export async function deleteCategoryBonusRate(category) {
  await sql`DELETE FROM category_bonus_rates WHERE category = ${category}`;
}

// Per-user × per-category bonus rate overrides (most specific layer).
export async function getUserCategoryBonusRates(username) {
  // Guard: table may not exist before /api/init → return no overrides.
  const { rows: t } = await sql`SELECT to_regclass('user_category_bonus_rates') AS t`;
  if (!t[0]?.t) return [];
  if (username) {
    const { rows } = await sql`SELECT * FROM user_category_bonus_rates WHERE username = ${username} ORDER BY category`;
    return rows;
  }
  const { rows } = await sql`SELECT * FROM user_category_bonus_rates ORDER BY username, category`;
  return rows;
}

/**
 * Upsert a per-user-per-category bonus rate. Blank fields = "no change"
 * (COALESCE keeps existing), same semantics as the other rate setters.
 * @param {{username:string, category:string, seller_fixed?:number, seller_percentage?:number, driver_fixed?:number, updatedBy:string}} data
 */
export async function setUserCategoryBonusRate(data) {
  const sf = data.seller_fixed != null ? parseFloat(data.seller_fixed) : null;
  const sp = data.seller_percentage != null ? parseFloat(data.seller_percentage) : null;
  const df = data.driver_fixed != null ? parseFloat(data.driver_fixed) : null;
  await sql`
    INSERT INTO user_category_bonus_rates (username, category, seller_fixed, seller_percentage, driver_fixed, updated_by, updated_at)
    VALUES (${data.username}, ${data.category}, ${sf}, ${sp}, ${df}, ${data.updatedBy}, NOW())
    ON CONFLICT (username, category) DO UPDATE SET
      seller_fixed = COALESCE(${sf}, user_category_bonus_rates.seller_fixed),
      seller_percentage = COALESCE(${sp}, user_category_bonus_rates.seller_percentage),
      driver_fixed = COALESCE(${df}, user_category_bonus_rates.driver_fixed),
      updated_by = ${data.updatedBy},
      updated_at = NOW()
  `;
}

export async function deleteUserCategoryBonusRate(username, category) {
  await sql`DELETE FROM user_category_bonus_rates WHERE username = ${username} AND category = ${category}`;
}
...
export async function calculateBonusInTx(client, saleId, deliveryId, driverUsername) {
  ...
  // Returns { flat, cat } for a user: `flat` = the across-all-categories
  // override (user_bonus_rates), `cat` = the override for THIS product's
  // category (user_category_bonus_rates, guarded so a not-yet-migrated env
  // simply has no per-category layer). Field precedence is applied by the
  // caller: cat > flat > category-rate > global.
  async function getUserRates(username, category, hasUserCatTable) {
    const { rows: flatRows } = await client.sql`
    ...
    let cat = null;
    if (hasUserCatTable) {
      const { rows: catRows } = await client.sql`
        SELECT seller_fixed, seller_percentage, driver_fixed
        FROM user_category_bonus_rates WHERE username = ${username} AND category = ${category}
      `;
      cat = catRows[0] || null;
    }
    return { flat, cat };
  ...
  // Per-category bonus rate. Same formula as before — only the rate SOURCE
  // changes: per-user override > per-category rate > global default. This
  // lets cheap categories (إكسسوارات، قطع تبديل) carry a low/zero fixed bonus
  // so the commission never exceeds the item's price. A missing category row
  // (or NULL column) falls through to the global value, so behavior is
  // unchanged until the admin configures a category.
  const { rows: prodCatRows } = await client.sql`
    SELECT category FROM products WHERE name = ${sale.item}
  `;
  const productCategory = prodCatRows[0]?.category || '';
  // Guard against a not-yet-migrated environment: if the category_bonus_rates
  // table doesn't exist yet (e.g. code deployed before initDatabase/migrate
  // ran), to_regclass returns NULL — tx-safe, no error — and we fall back to
  // the global rate instead of aborting the delivery-confirm transaction.
  let catRate = null;
  const { rows: catTblRows } = await client.sql`SELECT to_regclass('category_bonus_rates') AS t`;
  if (catTblRows[0]?.t) {
    const { rows: catRateRows } = await client.sql`
      SELECT seller_fixed, seller_percentage, driver_fixed
      FROM category_bonus_rates WHERE category = ${productCategory}
    `;
    catRate = catRateRows[0] || null;
  }
  // Existence guard for the per-user × per-category table (same tx-safe rationale).
  const { rows: userCatTblRows } = await client.sql`SELECT to_regclass('user_category_bonus_rates') AS t`;
  const userCatExists = !!userCatTblRows[0]?.t;
  ...
  if (sale.created_by) {
    ...
    if (sellerRole === 'seller') {
      ...
      const sellerRates = await getUserRates(sale.created_by, productCategory, userCatExists);
      // Precedence (per field): user+category > user-flat > category > global.
      const sellerFixed = sellerRates.cat?.seller_fixed != null
        ? parseFloat(sellerRates.cat.seller_fixed)
        : sellerRates.flat?.seller_fixed != null
        ? parseFloat(sellerRates.flat.seller_fixed)
        : catRate?.seller_fixed != null
        ? parseFloat(catRate.seller_fixed)
        : globalSellerFixed;
      const sellerPct = sellerRates.cat?.seller_percentage != null
        ? parseFloat(sellerRates.cat.seller_percentage)
        : sellerRates.flat?.seller_percentage != null
        ? parseFloat(sellerRates.flat.seller_percentage)
        : catRate?.seller_percentage != null
        ? parseFloat(catRate.seller_percentage)
        : globalSellerPct;
  ...
  if (driverUsername) {
    ...
    if (driverRole === 'driver') {
      ...
      const driverRates = await getUserRates(confirmedDriver, productCategory, userCatExists);
      // Precedence (per field): user+category > user-flat > category > global.
      const driverFixed = driverRates.cat?.driver_fixed != null
        ? parseFloat(driverRates.cat.driver_fixed)
        : driverRates.flat?.driver_fixed != null
        ? parseFloat(driverRates.flat.driver_fixed)
        : catRate?.driver_fixed != null
        ? parseFloat(catRate.driver_fixed)
        : globalDriverFixed;
 ...
 * @returns {Promise<void>}
 */
// Correct ONLY the VIN on an invoice. The driver often mis-enters it at
// delivery and it cannot otherwise be fixed. VIN lives in two places — the
// invoice snapshot (invoices.vin, what the list + PDF show) and the source
// sale (sales.vin) — so both are updated together to stay consistent. No
// other invoice field is touched.
export async function updateInvoiceVin(invoiceId, vin, updatedBy) {
  const v = (vin == null ? '' : String(vin)).trim();
  return withTx(async (client) => {
    const { rows } = await client.sql`SELECT sale_id FROM invoices WHERE id = ${invoiceId} FOR UPDATE`;
    if (!rows.length) throw new Error('الفاتورة غير موجودة');
    const saleId = rows[0].sale_id;
    await client.sql`UPDATE invoices SET vin = ${v}, updated_by = ${updatedBy || null}, updated_at = NOW() WHERE id = ${invoiceId}`;
    if (saleId) {
      await client.sql`UPDATE sales SET vin = ${v} WHERE id = ${saleId}`;
    }
    return { success: true, vin: v };
  });
}
...
export async function updateSale(data) {
  return withTx(async (client) => {
    ...
    // Only reserved (محجوز) orders are editable. The single post-delivery change
    // allowed is the VIN, handled separately via updateInvoiceVin. Corrections
    // to a delivered order go through cancellation/return, not editing.
    if (oldRows[0].status === 'ملغي') throw new Error('لا يمكن تعديل طلب ملغي');
    if (oldRows[0].status === 'مؤكد') {
      throw new Error('لا يمكن تعديل طلب مؤكد بعد التسليم');
    }
    ...
    // Mirror the change onto the linked delivery so the dashboard stays
    // consistent. The address lives on the delivery, so an address edit must
    // land here. COALESCE keeps the existing address when none was provided.
    await client.sql`
      UPDATE deliveries
      SET client_name = ${data.clientName},
          items = ${newItem + ' (' + newQty + ')'},
          total_amount = ${total},
          address = COALESCE(${data.clientAddress ?? null}, address)
...
export async function addSettlement(data) {
  ...
  return withTx(async (client) => {
    ...
    // Treasury shadow (no-op unless TREASURY_ENABLED): a commission payout is
    // paid from the shared general box (project funds) → debits the general box.
    if ((data.type === 'seller_payout' || data.type === 'driver_payout') && (parseFloat(data.amount) || 0) > 0) {
      await emitCashMovement(client, {
        general: true,
        signedAmount: -(parseFloat(data.amount) || 0),
        method: 'كاش',
...
export async function addProfitDistribution(data) {
  ...
  return withTx(async (client) => {
    ...
    for (const r of recipients) {
      ...
      const { rows: pdRows } = await client.sql`
        ...
        RETURNING id
      `;
      // Treasury shadow (no-op unless TREASURY_ENABLED): each recipient's share
      // is paid from the shared general box → debits the general box.
      await emitCashMovement(client, {
        general: true,
        signedAmount: -amount,
        method: 'كاش',
        kind: 'profit_distribution',
        sourceTable: 'profit_distributions',
        sourceId: pdRows[0].id,
        createdBy: data.createdBy || '',
      });
    }
    return {
