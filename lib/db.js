...
export async function emitCashMovement(client, opts) {
  if (process.env.TREASURY_ENABLED !== 'true') return;
...
export async function initiateHandoverInTx(client, data) {
  if (process.env.TREASURY_ENABLED !== 'true') throw new Error('نظام الصناديق غير مفعّل');
...
export async function updateUser(data) {
  ...
  if (process.env.TREASURY_ENABLED === 'true' && data.role) {
