...
export async function updateSale(data) {
  return withTx(async (client) => {
    const { rows: oldRows } = await client.sql`
      SELECT item, quantity, status, cost_price, paid_amount, payment_status FROM sales WHERE id = ${data.id} FOR UPDATE
    `;
...
export async function updatePurchase(data) {
  ...
  return withTx(async (client) => {
    ...
    // Stock adjustment if item or quantity changed
    ...
    if (newItem === oldItem && newQty !== oldQty) {
      ...
      // Item swapped: return old stock, add new stock
      if (oldQty > 0) {
        await client.sql`UPDATE products SET stock = stock - ${oldQty}::numeric WHERE name = ${oldItem}`;
      }
      if (newQty > 0) {
    ...
    // Update buy_price on the product if it changed
    if (newUnitPrice !== (parseFloat(old.unit_price) || 0)) {
      await client.sql`UPDATE products SET buy_price = ${newUnitPrice} WHERE name = ${newItem}`;
    }
    ...
    await client.sql`
      UPDATE purchases SET
        supplier = ${newSupplier}, item = ${newItem}, quantity = ${newQty},
