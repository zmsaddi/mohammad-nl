...
export async function voidInvoice(id) {
  // Refuse to void if any settled bonus is linked through this invoice's sale —
  // money has already been paid out, so the books would silently break.
  const { rows: inv } = await sql`SELECT sale_id FROM invoices WHERE id = ${id}`;
  if (!inv.length) throw new Error('الفاتورة غير موجودة');
  const { rows: settled } = await sql`
    SELECT 1 FROM bonuses WHERE sale_id = ${inv[0].sale_id} AND settled = true LIMIT 1
  `;
