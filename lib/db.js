...
export async function addSale(data) {
  ...
  const paymentType = data.paymentType || 'كاش'; // كاش / بنك / آجل
  ...
  const { rows } = await sql`
    INSERT INTO sales (date, client_name, item, quantity, cost_price, unit_price, total, cost_total, profit, payment_method, payment_type, paid_amount, remaining, status, ref_code, created_by, notes)
    VALUES (${data.date}, ${data.clientName}, ${data.item}, ${data.quantity}, ${costPrice}, ${sellPrice}, ${total}, ${costTotal}, ${profit}, ${paymentType}, ${paymentType}, ${0}, ${total}, 'محجوز', ${saleRef}, ${data.createdBy || ''}, ${data.notes || ''})
    RETURNING id
  `;
  ...
  const { rows: delRows } = await sql`
    ...
    VALUES (${data.date}, ${data.clientName}, ${data.clientPhone || ''}, ${data.clientEmail || ''}, ${data.clientAddress || ''}, ${data.item + ' (' + data.quantity + ')'}, ${total}, 'قيد الانتظار', '', ${delRef}, ${data.createdBy || ''}, ${'بيع رقم ' + saleId})
    RETURNING id, ref_code
  `;
...
export async function addExpense(data) {
  const { rows } = await sql`
    ...
    VALUES (${data.date}, ${data.category}, ${data.description}, ${data.amount}, ${data.paymentType || 'نقدي'}, ${data.createdBy || ''}, ${data.notes || ''})
    RETURNING id
  `;
  return rows[0].id;
}
...
export async function updateDelivery(data) {
  ...
  // DELIVERY CONFIRMED → Confirm sale + mark payment + calculate bonuses
...
export async function calculateBonus(saleId, deliveryId, driverUsername) {
  ...
  const { rows: prodRows } = await sql`SELECT sell_price FROM products WHERE name = ${sale.item}`;
  const recommended = prodRows.length > 0 ? (prodRows[0].sell_price || 0) : 0;
  ...
  // Seller bonus
  if (!existing.length && sale.created_by) {
  ...
  if (!driverExisting.length && driverUsername) {
    const today = new Date().toISOString().split('T')[0];
    await sql`
      INSERT INTO bonuses (date, username, role, sale_id, delivery_id, item, quantity, recommended_price, actual_price, fixed_bonus, extra_bonus, total_bonus)
...
export async function addSettlement(data) {
  ...
  // Mark bonuses as settled for this user
  if (data.username && (data.type === 'seller_payout' || data.type === 'driver_payout')) {
    await sql`UPDATE bonuses SET settled = true, settlement_id = ${settlementId} WHERE username = ${data.username} AND settled = false`;
  }
