...
export async function addPurchase(data) {
  ...
  const { rows } = await sql`
    INSERT INTO purchases (date, supplier, item, quantity, unit_price, total, payment_type, ref_code, notes)
    VALUES (${data.date}, ${data.supplier}, ${data.item}, ${data.quantity}, ${data.unitPrice}, ${total}, ${data.paymentType || 'نقدي'}, ${refCode}, ${data.notes || ''})
...
export async function addSale(data) {
  ...
  const { rows } = await sql`
    INSERT INTO sales (date, client_name, item, quantity, cost_price, unit_price, total, cost_total, profit, payment_method, payment_type, paid_amount, remaining, status, ref_code, notes)
    VALUES (${data.date}, ${data.clientName}, ${data.item}, ${data.quantity}, ${costPrice}, ${sellPrice}, ${total}, ${costTotal}, ${profit}, ${data.paymentMethod}, ${data.paymentType || 'نقدي'}, ${paid}, ${remaining}, 'محجوز', ${saleRef}, ${data.notes || ''})
    RETURNING id
  `;
  ...
  const { rows: delRows } = await sql`
    ...
    VALUES (${data.date}, ${data.clientName}, ${data.clientPhone || ''}, ${data.clientEmail || ''}, ${data.clientAddress || ''}, ${data.item + ' (' + data.quantity + ')'}, ${total}, 'قيد الانتظار', '', ${delRef}, ${'بيع رقم ' + saleId})
    RETURNING id, ref_code
  `;
...
export async function addClient(data) {
  ...
  const { rows } = await sql`
    ...
    VALUES (${data.name}, ${data.phone || ''}, ${data.address || ''}, ${data.email || ''}, ${data.notes || ''})
    RETURNING id
  `;
  return { id: rows[0].id };
}
