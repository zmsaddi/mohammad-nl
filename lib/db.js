...
export async function getProfitShareConfig() {
  const { rows } = await sql`
    SELECT username, name, role, profit_share_pct
    FROM users
...
/**
 * v1.2 — set a user's profit share percentage.
 * @param {string} username
 * @param {number} percentage  0-100
 */
export async function setProfitSharePct(username, percentage) {
  const pct = parseFloat(percentage) || 0;
  if (pct < 0 || pct > 100) throw new Error('النسبة يجب أن تكون بين 0 و 100');
  await sql`UPDATE users SET profit_share_pct = ${pct} WHERE username = ${username}`;
}
