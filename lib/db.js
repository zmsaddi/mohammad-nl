      ...
      if (b.settled) {
        ...
        await client.sql`
          ...
          VALUES (CURRENT_DATE, 'seller_payout', ${b.username},
            ${'استرداد بونص — طلب ملغي #' + saleId + ' — فاتورة ' + invoiceRef},
            ${amount}, ${cancelledBy},
            ${'بونص مسوَّى سابقاً تم استرداده بسبب إلغاء الطلب'})
        `;
      }
    }
...
export async function getDistributablePoolForPeriod(startDate, endDate) {
  // v1.2 — distributable = NET PROFIT (cash-basis), not raw collected.
  //
  // صافي الربح = محصّلات - تكلفة بضاعة مباعة(مدفوعة) - مصروفات - بونصات
  //
  // This is the correct accounting base for profit distribution.
  // Raw collected revenue includes money that covers COGS + expenses.
  const f = startDate || null;
  const t = endDate || null;

  // 1. Net collected (collections + refunds)
  const { rows: r1 } = await sql`
    SELECT COALESCE(SUM(CASE WHEN type='collection' THEN amount WHEN type='refund' THEN amount ELSE 0 END), 0) AS v
    FROM payments WHERE (${f}::text IS NULL OR date >= ${f}) AND (${t}::text IS NULL OR date <= ${t})
  `;
  const collected = parseFloat(r1[0].v) || 0;

  // 2. COGS of fully-paid confirmed sales in period
  const { rows: r2 } = await sql`
    SELECT COALESCE(SUM(cost_total), 0) AS v FROM sales
    WHERE status = 'مؤكد' AND (payment_status = 'paid' OR remaining::numeric < 0.005)
      AND (${f}::text IS NULL OR date >= ${f}) AND (${t}::text IS NULL OR date <= ${t})
  `;
  const cogs = parseFloat(r2[0].v) || 0;

  // 3. Expenses in period
  const { rows: r3 } = await sql`
    SELECT COALESCE(SUM(amount), 0) AS v FROM expenses
    WHERE (${f}::text IS NULL OR date >= ${f}) AND (${t}::text IS NULL OR date <= ${t})
  `;
  const expenses = parseFloat(r3[0].v) || 0;

  // 4. Bonus payouts (settlements) in period
  const { rows: r4 } = await sql`
    SELECT COALESCE(SUM(amount), 0) AS v FROM settlements
    WHERE type IN ('seller_payout','driver_payout')
      AND (${f}::text IS NULL OR date >= ${f}) AND (${t}::text IS NULL OR date <= ${t})
  `;
  const bonusPaid = parseFloat(r4[0].v) || 0;

  // 5. Already distributed — any distribution whose period OVERLAPS
  //    with the requested range. Pre-v1.2 this used exact match which
  //    missed distributions with slightly different dates.
  //
  //    Overlap logic: two ranges [A_start, A_end] and [B_start, B_end]
  //    overlap when NOT (A_end < B_start OR A_start > B_end).
  //    Null dates = "all time" → always overlaps.
  const { rows: r5 } = await sql`
    SELECT COALESCE(SUM(amount), 0) AS v FROM profit_distributions
    WHERE (
      -- null period on either side = all-time → always matches
      (${f}::text IS NULL OR base_period_end IS NULL OR base_period_end >= ${f})
      AND
      (${t}::text IS NULL OR base_period_start IS NULL OR base_period_start <= ${t})
    )
  `;
  const already_distributed = parseFloat(r5[0].v) || 0;

  const net_profit = collected - cogs - expenses - bonusPaid;
  ...
  return {
    ...
    net_profit,
    already_distributed,
    remaining: net_profit - already_distributed,
  };
}
...
export async function addProfitDistribution(data) {
  ...
  return withTx(async (client) => {
    ...
    //
    // صافي الربح = محصّلات - تكلفة بضاعة - مصروفات - بونصات - موزع سابقاً
    //
    // Pre-v1.2 this used raw collected revenue which let the admin
    // distribute money that covers COGS + expenses — not actual profit.

    // 3a. Net collected (collections + refunds)
    const { rows: cR } = await client.sql`
      SELECT COALESCE(SUM(CASE WHEN type='collection' THEN amount WHEN type='refund' THEN amount ELSE 0 END), 0) AS v
      FROM payments WHERE (${startDate}::text IS NULL OR date >= ${startDate}) AND (${endDate}::text IS NULL OR date <= ${endDate})
    `;
    const collected = parseFloat(cR[0].v) || 0;

    // 3b. COGS (fully-paid confirmed sales)
    const { rows: cogsR } = await client.sql`
      SELECT COALESCE(SUM(cost_total), 0) AS v FROM sales
      WHERE status = 'مؤكد' AND (payment_status = 'paid' OR remaining::numeric < 0.005)
        AND (${startDate}::text IS NULL OR date >= ${startDate}) AND (${endDate}::text IS NULL OR date <= ${endDate})
    `;
    const capCogs = parseFloat(cogsR[0].v) || 0;

    // 3c. Expenses
    const { rows: expR } = await client.sql`
      SELECT COALESCE(SUM(amount), 0) AS v FROM expenses
      WHERE (${startDate}::text IS NULL OR date >= ${startDate}) AND (${endDate}::text IS NULL OR date <= ${endDate})
    `;
    const capExpenses = parseFloat(expR[0].v) || 0;

    // 3d. Bonus payouts
    const { rows: bonR } = await client.sql`
      SELECT COALESCE(SUM(amount), 0) AS v FROM settlements
      WHERE type IN ('seller_payout','driver_payout')
        AND (${startDate}::text IS NULL OR date >= ${startDate}) AND (${endDate}::text IS NULL OR date <= ${endDate})
    `;
    const capBonusPaid = parseFloat(bonR[0].v) || 0;

    // 3e. Already distributed — OVERLAP check (not exact match)
    const { rows: distRows } = await client.sql`
      SELECT COALESCE(SUM(amount), 0) AS already FROM profit_distributions
      WHERE (
        (${startDate}::text IS NULL OR base_period_end IS NULL OR base_period_end >= ${startDate})
        AND
        (${endDate}::text IS NULL OR base_period_start IS NULL OR base_period_start <= ${endDate})
      )
    `;
    const alreadyDistributed = parseFloat(distRows[0].already) || 0;

    const netProfit = collected - capCogs - capExpenses - capBonusPaid;
    const cap = netProfit - alreadyDistributed;
    if (baseAmount > cap + 0.01) {
      ...
      throw new Error(
        ...
        `محصّلات=${collected.toFixed(2)}€، تكلفة=${capCogs.toFixed(2)}€، مصروفات=${capExpenses.toFixed(2)}€، بونصات=${capBonusPaid.toFixed(2)}€، موزع سابقاً=${alreadyDistributed.toFixed(2)}€`
      );
    }
