 ...
 * deleteDelivery). Runs inside the caller's withTx client — everything
...
export async function cancelSale(client, options) {
  ...
  // Step 6: restore product stock (skip if the sale was already cancelled
  // — prevents double-restore if cancelSale is run twice on the same sale)
  if (!alreadyCancelled) {
...
export async function getClients(withDebt = false) {
  ...
  return clients.map((client) => {
    ...
    // ARC-06: @vercel/postgres returns NUMERIC columns as strings. The naive
    // pattern `sum + (s.total || 0)` would start with 0 (number) + "100.00"
    // (string) and produce "0100.00" via string concatenation. Wrap every
    // read of a money column in parseFloat to force ToNumber coercion.
    const totalSales = clientSales.reduce((sum, s) => sum + (parseFloat(s.total) || 0), 0);
    ...
    const totalLaterPayments = clientPayments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0);

    const totalPaid = clientSales
      .filter((s) => ['كاش', 'بنك'].includes(s.payment_type) && s.status === 'مؤكد')
      .reduce((sum, s) => sum + (parseFloat(s.total) || 0), 0) + totalPaidAtSale + totalLaterPayments;
 ...
 *
 * Naming note: the function is still called `deleteDelivery` for API
 * backwards-compat, but it no longer physically removes the delivery
 * row. The row is preserved with status='ملغي' as audit history, same
 * way updateDelivery(cancel) and voidInvoice leave their data in place.
 *
 * The reason the row is preserved: `invoices.delivery_id` has a NOT NULL
 * FK with ON DELETE CASCADE ([lib/db.js:289](lib/db.js#L289)). If we
 ...
 * @returns {Promise<void>}
 */
export async function deleteDelivery(id, options = {}) {
  return withTx(async (client) => {
    const { rows: delRows } = await client.sql`
...
export async function getSummaryData(from, to) {
  ...
  purchases.forEach((p) => {
    ...
    supplierMap[p.supplier].totalSpent += parseFloat(p.total) || 0;
