...
export async function initDatabase() {
  ...
  await sql`ALTER TABLE purchases ADD COLUMN IF NOT EXISTS paid_amount NUMERIC(19,2) DEFAULT 0`.catch(() => {});
  await sql`ALTER TABLE purchases ADD COLUMN IF NOT EXISTS payment_status TEXT DEFAULT 'paid'`.catch(() => {});
  ...
  await sql`UPDATE purchases SET paid_amount = total WHERE paid_amount = 0 AND total > 0`.catch(() => {});
  await sql`UPDATE purchases SET payment_status = CASE WHEN paid_amount >= total THEN 'paid' WHEN paid_amount > 0 THEN 'partial' ELSE 'pending' END`.catch(() => {});
  ...
  `.catch(() => {});
  await sql`CREATE INDEX IF NOT EXISTS supplier_payments_purchase_id_idx ON supplier_payments(purchase_id)`.catch(() => {});
  ...
  `.catch(() => {});
  await sql`CREATE INDEX IF NOT EXISTS profit_distributions_group_idx ON profit_distributions(group_id)`.catch(() => {});
  await sql`CREATE INDEX IF NOT EXISTS profit_distributions_username_idx ON profit_distributions(username)`.catch(() => {});
  await sql`CREATE INDEX IF NOT EXISTS profit_distributions_created_idx ON profit_distributions(created_at DESC)`.catch(() => {});
  ...
  await sql`ALTER TABLE suppliers DROP CONSTRAINT IF EXISTS suppliers_name_key`.catch(() => {});
  await sql`
  ...
  //
  // Each ALTER is wrapped in .catch(() => {}) so re-running initDatabase
  // after the migration succeeds is a silent no-op (the column is already
  // NUMERIC and Postgres returns "type is already the desired type").
  //
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // purchases
  await sql`ALTER TABLE purchases ALTER COLUMN unit_price TYPE NUMERIC(19,2) USING unit_price::numeric`.catch(() => {});
  await sql`ALTER TABLE purchases ALTER COLUMN total      TYPE NUMERIC(19,2) USING total::numeric`.catch(() => {});
  // sales (inline columns + recommended_price from an earlier ALTER)
  await sql`ALTER TABLE sales ALTER COLUMN cost_price        TYPE NUMERIC(19,2) USING cost_price::numeric`.catch(() => {});
  await sql`ALTER TABLE sales ALTER COLUMN unit_price        TYPE NUMERIC(19,2) USING unit_price::numeric`.catch(() => {});
  await sql`ALTER TABLE sales ALTER COLUMN total             TYPE NUMERIC(19,2) USING total::numeric`.catch(() => {});
  await sql`ALTER TABLE sales ALTER COLUMN cost_total        TYPE NUMERIC(19,2) USING cost_total::numeric`.catch(() => {});
  await sql`ALTER TABLE sales ALTER COLUMN profit            TYPE NUMERIC(19,2) USING profit::numeric`.catch(() => {});
  await sql`ALTER TABLE sales ALTER COLUMN paid_amount       TYPE NUMERIC(19,2) USING paid_amount::numeric`.catch(() => {});
  await sql`ALTER TABLE sales ALTER COLUMN remaining         TYPE NUMERIC(19,2) USING remaining::numeric`.catch(() => {});
  await sql`ALTER TABLE sales ALTER COLUMN quantity          TYPE NUMERIC(19,2) USING quantity::numeric`.catch(() => {});
  await sql`ALTER TABLE sales ALTER COLUMN recommended_price TYPE NUMERIC(19,2) USING recommended_price::numeric`.catch(() => {});
  // expenses
  await sql`ALTER TABLE expenses ALTER COLUMN amount TYPE NUMERIC(19,2) USING amount::numeric`.catch(() => {});
  // payments
  await sql`ALTER TABLE payments ALTER COLUMN amount TYPE NUMERIC(19,2) USING amount::numeric`.catch(() => {});
  // products
  await sql`ALTER TABLE products ALTER COLUMN buy_price  TYPE NUMERIC(19,2) USING buy_price::numeric`.catch(() => {});
  await sql`ALTER TABLE products ALTER COLUMN sell_price TYPE NUMERIC(19,2) USING sell_price::numeric`.catch(() => {});
  await sql`ALTER TABLE products ALTER COLUMN stock      TYPE NUMERIC(19,2) USING stock::numeric`.catch(() => {});
  // deliveries
  await sql`ALTER TABLE deliveries ALTER COLUMN total_amount TYPE NUMERIC(19,2) USING total_amount::numeric`.catch(() => {});
  // bonuses
  await sql`ALTER TABLE bonuses ALTER COLUMN quantity          TYPE NUMERIC(19,2) USING quantity::numeric`.catch(() => {});
  await sql`ALTER TABLE bonuses ALTER COLUMN recommended_price TYPE NUMERIC(19,2) USING recommended_price::numeric`.catch(() => {});
  await sql`ALTER TABLE bonuses ALTER COLUMN actual_price      TYPE NUMERIC(19,2) USING actual_price::numeric`.catch(() => {});
  await sql`ALTER TABLE bonuses ALTER COLUMN fixed_bonus       TYPE NUMERIC(19,2) USING fixed_bonus::numeric`.catch(() => {});
  await sql`ALTER TABLE bonuses ALTER COLUMN extra_bonus       TYPE NUMERIC(19,2) USING extra_bonus::numeric`.catch(() => {});
  await sql`ALTER TABLE bonuses ALTER COLUMN total_bonus       TYPE NUMERIC(19,2) USING total_bonus::numeric`.catch(() => {});
  // settlements
  await sql`ALTER TABLE settlements ALTER COLUMN amount TYPE NUMERIC(19,2) USING amount::numeric`.catch(() => {});
  // price_history
  await sql`ALTER TABLE price_history ALTER COLUMN old_buy_price  TYPE NUMERIC(19,2) USING old_buy_price::numeric`.catch(() => {});
  await sql`ALTER TABLE price_history ALTER COLUMN new_buy_price  TYPE NUMERIC(19,2) USING new_buy_price::numeric`.catch(() => {});
  await sql`ALTER TABLE price_history ALTER COLUMN old_sell_price TYPE NUMERIC(19,2) USING old_sell_price::numeric`.catch(() => {});
  await sql`ALTER TABLE price_history ALTER COLUMN new_sell_price TYPE NUMERIC(19,2) USING new_sell_price::numeric`.catch(() => {});
  // invoices
  await sql`ALTER TABLE invoices ALTER COLUMN quantity   TYPE NUMERIC(19,2) USING quantity::numeric`.catch(() => {});
  await sql`ALTER TABLE invoices ALTER COLUMN unit_price TYPE NUMERIC(19,2) USING unit_price::numeric`.catch(() => {});
  await sql`ALTER TABLE invoices ALTER COLUMN total      TYPE NUMERIC(19,2) USING total::numeric`.catch(() => {});
  ...
  await sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS down_payment_expected NUMERIC(19,2) DEFAULT 0`.catch(() => {});
  await sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_status        TEXT          DEFAULT 'pending'`.catch(() => {});
  await sql`
  ...
  `.catch(() => {});
  await sql`CREATE INDEX IF NOT EXISTS sales_payment_status_idx ON sales(payment_status)`.catch(() => {});

  // payments: distinguish collection vs refund, track payment method + TVA
  await sql`ALTER TABLE payments ADD COLUMN IF NOT EXISTS type           TEXT         DEFAULT 'collection'`.catch(() => {});
  await sql`ALTER TABLE payments ADD COLUMN IF NOT EXISTS payment_method TEXT         DEFAULT 'كاش'`.catch(() => {});
  await sql`ALTER TABLE payments ADD COLUMN IF NOT EXISTS tva_amount     NUMERIC(19,2) DEFAULT 0`.catch(() => {});
  await sql`
  ...
  `.catch(() => {});
  await sql`CREATE INDEX IF NOT EXISTS cancellations_sale_id_idx ON cancellations(sale_id)`.catch(() => {});
...
export async function cancelSale(client, options) {
  ...
  // Step 3: settled-bonus block (role-specific messages per the role/bonus audit)
  const sellerSettled = sellerBonuses.some((b) => b.settled);
  const driverSettled = driverBonuses.some((b) => b.settled);
  if (sellerSettled && driverSettled) {
    const err = new Error(
      'لا يمكن إلغاء هذه العملية. مكافأة البائع والسائق دُفعتا بالفعل. يجب إلغاء التسويات أولاً.'
    );
  ...
  if (sellerSettled) {
    const err = new Error(
      'لا يمكن إلغاء هذه العملية. مكافأة البائع دُفعت بالفعل. يجب إلغاء التسوية أولاً.'
    );
  ...
  if (driverSettled) {
    const err = new Error(
      'لا يمكن إلغاء هذه العملية. مكافأة السائق دُفعت بالفعل. يجب إلغاء التسوية أولاً.'
    );
    err.code = 'SETTLED_BONUS_DRIVER';
    throw err;
  }

  // Gather collection payments (needed for refund amount + preview)
  const { rows: paymentRows } = await client.sql`
    SELECT id, amount, tva_amount, payment_method, type
    FROM payments WHERE sale_id = ${saleId} AND type = 'collection'
  `;
  ...
  if (!sellerKept && sellerBonuses.length > 0) {
    await client.sql`
      DELETE FROM bonuses WHERE sale_id = ${saleId} AND role = 'seller' AND settled = false
    `;
  }
  if (!driverKept && driverBonuses.length > 0) {
    await client.sql`
      DELETE FROM bonuses WHERE sale_id = ${saleId} AND role = 'driver' AND settled = false
    `;
  }
...
export async function applyCollectionInTx(client, saleId, amountTTC, paymentMethod, collectedBy) {
  ...
  // Step 4: proportional TVA (20% flat → TTC / 6)
  const tva = Math.round((amount / 6) * 100) / 100;
...
export async function getDeliveries(status, assignedDriver, createdBy) {
  ...
  if (status && assignedDriver) {
    const { rows } = await sql`
      ...
      WHERE d.status = ${status} AND d.assigned_driver = ${assignedDriver}
      ORDER BY d.id DESC`;
    return rows;
  }
  if (status && createdBy) {
...
export async function updateDelivery(data) {
  return withTx(async (client) => {
    ...
    if (data.status === 'تم التوصيل') {
      ...
      // TVA is proportional — 20% TTC rate → amount_ttc / 6.
      if (dpe > 0.005) {
        const collectionMethod = sale.payment_type === 'بنك' ? 'بنك' : 'كاش';
        const tvaAmount = Math.round((dpe / 6) * 100) / 100;
...
export async function getSummaryData(from, to) {
  ...
  const totalCreditSales = sales.filter((s) => s.payment_type === 'آجل' && s.status === 'مؤكد').reduce((s, r) => s + (parseFloat(r.total) || 0), 0);
  const totalPaidAtSale = sales.filter((s) => s.payment_type === 'آجل' && s.status === 'مؤكد').reduce((s, r) => s + (parseFloat(r.paid_amount) || 0), 0);
  const totalLaterPayments = payments.reduce((s, p) => s + (parseFloat(p.amount) || 0), 0);
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const mp = purchases.filter((r) => r.date?.startsWith(ym)).reduce((s, r) => s + (parseFloat(r.total) || 0), 0);
    const ms = confirmedSales.filter((r) => r.date?.startsWith(ym)).reduce((s, r) => s + (parseFloat(r.total) || 0), 0);
    const me = expenses.filter((r) => r.date?.startsWith(ym)).reduce((s, r) => s + (parseFloat(r.amount) || 0), 0);
    const mProfit = confirmedSales.filter((r) => r.date?.startsWith(ym)).reduce((s, r) => s + (parseFloat(r.profit) || 0), 0);
  ...
  sales.filter((s) => s.payment_type === 'آجل').forEach((s) => {
    ...
    clientDebts[s.client_name].credit += parseFloat(s.total) || 0;
    clientDebts[s.client_name].paidAtSale += parseFloat(s.paid_amount) || 0;
  });
  payments.forEach((p) => {
    if (!clientDebts[p.client_name]) clientDebts[p.client_name] = { credit: 0, paidAtSale: 0, laterPaid: 0 };
    clientDebts[p.client_name].laterPaid += parseFloat(p.amount) || 0;
  });
  ...
  const netProfitCashBasis = grossProfitCashBasis - totalExpenses - totalBonusCost;

  // Pending collections (TTC + VAT share)
  const pendingRevenue = partialSales.reduce((s, r) => s + (parseFloat(r.remaining) || 0), 0);
  const pendingTva = Math.round((pendingRevenue / 6) * 100) / 100;
  ...
  return withTx(async (client) => {
    ...
    if (data.username && (data.type === 'seller_payout' || data.type === 'driver_payout')) {
      ...
      const { rows: unsettled } = await client.sql`
        ...
        FOR UPDATE
      `;
      const available = unsettled.reduce(
        (sum, b) => sum + (parseFloat(b.total_bonus) || 0),
        0
      );
      const requested = parseFloat(data.amount) || 0;
      // 1 cent tolerance for rounding drift between UI and DB
...
export async function addProfitDistribution(data) {
  ...
  // Eligibility check — pre-validate every username once before we
  // start inserting, so a row in the middle of the group can't fail
  // after half the recipients have been written.
  ...
  return withTx(async (client) => {
    ...
    for (const r of recipients) {
      const pct = parseFloat(r.percentage) || 0;
      // Compute each recipient's cut to 2 decimal places so the DB
      // value matches what the UI shows.
      ...
      await client.sql`
        INSERT INTO profit_distributions (
          ...
          ${data.basePeriodStart || null}, ${data.basePeriodEnd || null},
...
export async function addAlias(entityType, entityId, alias, normalizedAlias, source) {
  try {
    ...
    // BUG-07: log but swallow — AI layer must not break the voice flow
