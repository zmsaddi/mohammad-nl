...
export async function saveAICorrection(data) {
  try {
    ...
    const { rows: existing } = await sql`
      SELECT id, frequency FROM ai_patterns WHERE spoken_text = ${data.aiValue} AND correct_value = ${data.userValue} AND field_name = ${data.fieldName}
    `;
    if (existing.length > 0) {
      ...
      await sql`
        ...
        VALUES (${data.actionType}, ${data.aiValue}, ${data.userValue}, ${data.fieldName}, 1)
      `;
    }
    // Auto-create entity alias for name fields
    const nameFields = { client_name: 'client', supplier: 'supplier', item: 'product' };
    if (nameFields[data.fieldName] && data.userValue) {
