...
//
// Pre-v1.1 initDatabase() wrapped ~128 ALTER/CREATE calls in
// `.catch(() => {})` which silently swallowed every failure. A typo in
// a column name produced an invisible schema drift; a broken migration
// shipped undetected because every error was absorbed.
//
// This helper replaces the silent swallow with a targeted rethrow:
// only Postgres "already exists" / "duplicate" / "does not exist"
// class errors (the expected idempotent-DDL outcomes) are absorbed;
// everything else propagates to initDatabase which crashes the bootstrap
// loudly.
//
// Recognized idempotent patterns:
//   - "already exists" (CREATE TABLE / CREATE INDEX / CREATE CONSTRAINT
//     on pre-existing object)
//   - "duplicate column"
//   - "duplicate key"
//   - "relation .* already exists"
//   - "column .* of relation .* already exists"
//   - "does not exist" (used by some DROP IF EXISTS patterns)
//   - "multiple primary keys" (re-running CREATE TABLE)
//
// Any other error — syntax errors, FK violations, permission denied,
// network hiccups — rethrows.
function ignoreExpectedDdl(err) {
  ...
  if (
    ...
    // Neon pooler can route two sequential ALTER TABLE statements to
    // different Postgres backends which then cross-deadlock on FK-related
    // tables. Since every DDL here is idempotent (IF NOT EXISTS), a
    // deadlocked statement that doesn't execute is operationally the
    // same as "already exists" — the other process's version took
    // effect. Proper fix (Sprint 4 S4.1): run all initDatabase DDL
    // through a single non-pooled db.connect() connection. Until then,
    // absorb the transient deadlock.
    /deadlock detected/i.test(msg)
  ) {
    return; // swallow expected idempotent outcome
  }
  // Unexpected — rethrow so initDatabase crashes and the operator sees it.
...
async function withTx(fn) {
  ...
  try {
    ...
    // Swallow ROLLBACK failures — if the session is already dead the
    // rollback will error and we don't want to shadow the real `err`.
    // NOT a DDL catch; do not replace with ignoreExpectedDdl.
...
export async function getSummaryData(from, to) {
  ...
  const netProfitCashBasis = grossProfitCashBasis - totalExpenses - totalBonusCost - totalProfitDistributed;
...
export async function getDistributablePoolForPeriod(startDate, endDate) {
  const { rows: collRows } = await sql`
    SELECT COALESCE(SUM(
      CASE WHEN type = 'collection' THEN amount
    ...
    WHERE (${startDate}::text IS NULL OR date >= ${startDate})
      AND (${endDate}::text   IS NULL OR date <= ${endDate})
  `;
  const total_collected = parseFloat(collRows[0].net_collected) || 0;

  const { rows: distRows } = await sql`
    SELECT COALESCE(SUM(amount), 0) AS already
    FROM profit_distributions
    WHERE (
      (${startDate}::text IS NULL AND base_period_start IS NULL)
      OR base_period_start = ${startDate}
    )
    AND (
      (${endDate}::text IS NULL AND base_period_end IS NULL)
      OR base_period_end = ${endDate}
    )
  `;
  const already_distributed = parseFloat(distRows[0].already) || 0;

  return {
    total_collected,
    already_distributed,
    remaining: total_collected - already_distributed,
  };
}
...
export async function addProfitDistribution(data) {
  ...
  return withTx(async (client) => {
    ...
    // ── v1.1 F-001 STEP 3 — compute the remaining distributable pool
    //
    // collected   = Σ collection payments in period − Σ refund payments
    // distributed = Σ existing profit_distributions.amount for same period
    // cap         = collected − distributed
    // rejected if baseAmount > cap
    //
    // Refunds are counted as negative collections so cancelled sales
    // don't inflate the distributable base. Per the study §1.5 the
    // cancellation code inserts type='refund' payment rows with
    // negative amounts, so the single SELECT below captures both
    // directions cleanly.
    const { rows: collRows } = await client.sql`
      SELECT COALESCE(SUM(
        CASE
      ...
      WHERE (${startDate}::text IS NULL OR date >= ${startDate})
        AND (${endDate}::text   IS NULL OR date <= ${endDate})
    `;
    const collected = parseFloat(collRows[0].net_collected) || 0;

    // Existing distributions for the EXACT SAME period. We match on
    // the pair (start, end) — overlap handling is intentionally strict
    // here; v1.1 Phase B (parent/child schema) will generalize to
    // interval overlap. For now, two distributions with different
    // period tuples contend at different lock keys and are independent.
    const { rows: distRows } = await client.sql`
      SELECT COALESCE(SUM(amount), 0) AS already
      FROM profit_distributions
      WHERE (
        (${startDate}::text IS NULL AND base_period_start IS NULL)
    ...
    `;
    const alreadyDistributed = parseFloat(distRows[0].already) || 0;

    const cap = collected - alreadyDistributed;
    // 1 cent tolerance matches the rest of the codebase's rounding
    // discipline (settlements use the same ± 0.01 gate, see L3414).
    if (baseAmount > cap + 0.01) {
      const fmtPeriod = startDate && endDate ? `${startDate}..${endDate}` : '(كل الفترات)';
      throw new Error(
        `المبلغ المطلوب (${baseAmount.toFixed(2)}€) يتجاوز المبلغ المتاح للتوزيع ` +
        `(${cap.toFixed(2)}€) للفترة ${fmtPeriod}. ` +
        `المحصل=${collected.toFixed(2)}€، الموزع سابقاً=${alreadyDistributed.toFixed(2)}€`
      );
    }

    // ── v1.1 F-001 STEP 4 — actually insert the recipient rows.
    ...
    return {
      ...
      // v1.1 diagnostics — expose the cap math so the UI can render it
      // in the success toast and future tests can assert on it.
      cap_collected: collected,
      cap_already_distributed: alreadyDistributed,
