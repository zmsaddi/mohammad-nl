...
export async function deleteSale(id) {
  return withTx(async (client) => {
    ...
    if (saleData[0].status !== 'ملغي') {
      ...
      if (qty > 0) {
        await client.sql`UPDATE products SET stock = stock + ${qty}::numeric WHERE name = ${saleData[0].item}`;
      }
    }
  ...
  return withTx(async (client) => {
    ...
    if (data.status === 'ملغي') {
      ...
      if (saleRows.length && saleRows[0].status !== 'ملغي') {
        ...
        if (qty > 0) {
          await client.sql`UPDATE products SET stock = stock + ${qty}::numeric WHERE name = ${saleRows[0].item}`;
        }
