...
export async function initDatabase() {
  ...
  // Per-category bonus rate overrides — parallels user_bonus_rates but keyed
  // on the product's category. Resolution in calculateBonusInTx is:
  //   per-user override > per-category rate > global settings default.
  // A NULL column falls back to the next level, so a partially-filled row
  // (e.g. only seller_fixed set) is valid. Lets the admin set a low/zero
  // fixed bonus for cheap categories (إكسسوارات، قطع تبديل) so the commission
  // never exceeds the item's price, while دراجات keep the standard rate.
  await sql`
    CREATE TABLE IF NOT EXISTS category_bonus_rates (
  ...
  // Per-user × per-category bonus rate overrides — the most specific layer.
  // Resolution precedence in calculateBonusInTx becomes:
  //   (user+category) > (user flat, user_bonus_rates) > category > global.
  // Additive only: the flat user_bonus_rates table is untouched and still
  // acts as the user's default across categories. A NULL column falls
  // through to the next level.
  await sql`
    CREATE TABLE IF NOT EXISTS user_category_bonus_rates (
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // TREASURY / CASH-CUSTODY (Phase 1 — P0: schema only, INERT).
  //
  // These three tables are created EMPTY and are NOT read or written by any
  // existing code path. No boxes are seeded, no flag is wired, and no money
  // flow touches them yet — so creating them is a pure additive, zero-impact
  // change. Seeding, the TREASURY_ENABLED flag, and the read/write wiring
  // arrive in later phases (P1+) once the seeding/main-owner decisions are
  // confirmed. cash_boxes is created first because the other two FK to it.
  // ═══════════════════════════════════════════════════════════════════════════
  await sql`
    CREATE TABLE IF NOT EXISTS cash_boxes (
      id SERIAL PRIMARY KEY,
      owner_username TEXT UNIQUE NOT NULL REFERENCES users(username) ON DELETE RESTRICT,
      type TEXT NOT NULL DEFAULT 'custody' CHECK (type IN ('main','custody')),
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS cash_movements (
      ...
      box_id INTEGER NOT NULL REFERENCES cash_boxes(id),
      counterparty_box_id INTEGER REFERENCES cash_boxes(id),
      signed_amount NUMERIC(19,2) NOT NULL,
      method TEXT NOT NULL DEFAULT 'كاش' CHECK (method IN ('كاش','بنك')),
  ...
  `.catch(ignoreExpectedDdl);
  // Idempotent 1:1 emission guard — one movement row per (source row, kind, box).
  await sql`
    CREATE UNIQUE INDEX IF NOT EXISTS cash_movements_source_unique
    ON cash_movements(source_table, source_id, kind, box_id)
    WHERE source_id IS NOT NULL
  `.catch(ignoreExpectedDdl);
  await sql`CREATE INDEX IF NOT EXISTS cash_movements_box_idx ON cash_movements(box_id)`.catch(ignoreExpectedDdl);

  await sql`
    CREATE TABLE IF NOT EXISTS cash_handovers (
      id SERIAL PRIMARY KEY,
      kind TEXT NOT NULL DEFAULT 'handover' CHECK (kind IN ('handover','funding')),
      from_box_id INTEGER NOT NULL REFERENCES cash_boxes(id),
      to_box_id INTEGER NOT NULL REFERENCES cash_boxes(id),
  ...
  // Capital operations (external money IN / OUT of the project's general box).
  // Unlike internal transfers (dual-confirm handover), these require ALL active
  // admins to approve before the money moves. With a single admin, the
  // initiator's own approval is enough → applied immediately.
  await sql`
    CREATE TABLE IF NOT EXISTS capital_operations (
      id SERIAL PRIMARY KEY,
      kind TEXT NOT NULL CHECK (kind IN ('injection','withdrawal')),
      amount NUMERIC(19,2) NOT NULL CHECK (amount > 0),
      method TEXT NOT NULL DEFAULT 'كاش' CHECK (method IN ('كاش','بنك')),
  ...
  `.catch(ignoreExpectedDdl);
  await sql`
    CREATE TABLE IF NOT EXISTS capital_op_approvals (
      operation_id INTEGER NOT NULL REFERENCES capital_operations(id) ON DELETE CASCADE,
      username TEXT NOT NULL,
      created_at TIMESTAMPTZ DEFAULT NOW(),
  ...
  // Bank-receipt confirmation: for بنك sales an admin/manager must confirm the
  // funds arrived BEFORE delivery. These record who confirmed and when; the
  // delivery-confirm gate refuses a بنك sale until bank_received_by is set.
  await sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS bank_received_by TEXT`.catch(ignoreExpectedDdl);
  await sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS bank_received_at TIMESTAMPTZ`.catch(ignoreExpectedDdl);
  ...
  // v1.2 — profit_distribution_groups (parent table for FEAT-02 v2 schema).
  // Mirrors scripts/migrations/v1-2/001-profit-distribution-groups.sql so
  // fresh deployments don't need a manual `npm run migrate` step. The 3 items
  // (table + unique index + FK with VALIDATE) MUST land together — production
  // already has all three from a manual application; this just closes the
  // bootstrap gap so /api/init alone is sufficient on a clean Neon branch.
  await sql`
  ...
  `.catch(ignoreExpectedDdl);
  // Unique partial index — DB-level backstop for same-period duplicate
  // distributions. Only enforced when both dates are non-null (the F-001
  // application-layer cap covers the null-period case).
  await sql`
  ...
  `.catch(ignoreExpectedDdl);
  // FK from profit_distributions.group_id → profit_distribution_groups.id.
  // NOT VALID at creation; VALIDATE in a separate step so the DDL is safe
  // on existing rows even if any predate this constraint.
  await sql`
  ...
  // Treasury master switch (admin toggles it from the UI). Default OFF — the
  // whole cash-custody system stays inert until deliberately enabled.
  await sql`INSERT INTO settings (key, value) VALUES ('treasury_enabled', 'false') ON CONFLICT (key) DO NOTHING`.catch(ignoreExpectedDdl);
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // TREASURY (Phase 1) — schema finalization + box seeding. Runs AFTER users
  // exist. Still INERT: no money flow reads/writes these boxes yet.
  //
  // Model: ONE shared GENERAL box (owner_username NULL = collective project
  // capital, no single owner) + a personal custody box per cash-handling user
  // (admin/manager/driver), each separated. Sellers get no box.
  // ═══════════════════════════════════════════════════════════════════════════
  // owner_username must be nullable for the shared general box; drop the P0
  // single-column UNIQUE and enforce per-user uniqueness + a single 'main' via
  // partial indexes instead.
  await sql`ALTER TABLE cash_boxes ALTER COLUMN owner_username DROP NOT NULL`.catch(ignoreExpectedDdl);
  await sql`ALTER TABLE cash_boxes DROP CONSTRAINT IF EXISTS cash_boxes_owner_username_key`.catch(ignoreExpectedDdl);
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS cash_boxes_owner_unique ON cash_boxes(owner_username) WHERE owner_username IS NOT NULL`.catch(ignoreExpectedDdl);
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS cash_boxes_one_main ON cash_boxes((type)) WHERE type = 'main'`.catch(ignoreExpectedDdl);
  // Add the capital_withdrawal kind (capital OUT of the project, all-admin approved).
  await sql`ALTER TABLE cash_movements DROP CONSTRAINT IF EXISTS cash_movements_kind_check`.catch(ignoreExpectedDdl);
  await sql`
    ALTER TABLE cash_movements ADD CONSTRAINT cash_movements_kind_check
    CHECK (kind IN ('opening','capital_injection','capital_withdrawal','collection','refund',
                    'supplier_payment','expense','commission_payout','profit_distribution',
                    'funding','handover')) NOT VALID
  `.catch(ignoreExpectedDdl);
  // Seed the one shared general (main) box.
  await sql`
    INSERT INTO cash_boxes (owner_username, type)
    SELECT NULL, 'main'
    WHERE NOT EXISTS (SELECT 1 FROM cash_boxes WHERE type = 'main')
  `.catch(ignoreExpectedDdl);
  // Seed a custody box per existing cash-handling user.
  await sql`
    INSERT INTO cash_boxes (owner_username, type)
    SELECT u.username, 'custody' FROM users u
    WHERE u.role IN ('admin','manager','driver')
      AND NOT EXISTS (SELECT 1 FROM cash_boxes b WHERE b.owner_username = u.username)
  `.catch(ignoreExpectedDdl);
  // Backfill: a custody box belonging to a NON-cash role (e.g. a driver who was
  // later changed to a seller) should be deactivated so it drops out of the
  // money views. MONEY-SAFE: only EMPTY boxes are touched — a box that still
  // holds cash stays active and visible so its balance is never hidden.
  await sql`
    UPDATE cash_boxes SET active = false
    WHERE type = 'custody' AND active = true
      AND ABS(COALESCE(balance, 0)) <= 0.005
      AND owner_username IN (
        SELECT username FROM users WHERE role NOT IN ('admin','manager','driver')
      )
  `.catch(ignoreExpectedDdl);
