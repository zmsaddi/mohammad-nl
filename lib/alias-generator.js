...
const COLOR_WORDS = {
  ...
  'orange': ['برتقالي'],
  'brown':  ['بني', 'براون'],
  // French (Vitesse Eco's catalog uses French color suffixes like "- Noir")
  'noir':   ['نوار', 'أسود'],
  'blanc':  ['بلون', 'أبيض'],
  'bleu':   ['بلو', 'أزرق'],
  'rouge':  ['روج', 'أحمر'],
...
export function generateProductAliases(name) {
  ...
  const tokenInfos = tokens.map(tok => {
    ...
    // Try product/color/variant dictionaries
    ...
    // Try single-letter fallback (so "X" in "Sur-Ron Light Bee X" resolves)
    if (/^[A-Za-z]$/.test(tok)) {
      const ar = LETTER_PAIRS[tok.toUpperCase()];
      if (ar) return { arabicForms: [ar], latin: tok };
    }
    // Unknown — keep as-is in both positions
