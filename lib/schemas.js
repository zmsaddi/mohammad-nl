...
// Transaction dates may be today or earlier, never the future. getTodayDate()
// is the business-timezone (Europe/Amsterdam) "today", evaluated per request,
// so a same-day entry is always accepted while a future date is rejected. This
// shared schema enforces it on every endpoint that records a dated row
// (sales, purchases, expenses, deliveries, payments, settlements, ...).
const dateStr = z
  .string({ required_error: 'التاريخ مطلوب' })
  .regex(/^\d{4}-\d{2}-\d{2}$/, 'صيغة التاريخ غير صحيحة (YYYY-MM-DD)')
  .refine((d) => d <= getTodayDate(), { message: 'لا يمكن تسجيل تاريخ في المستقبل — اختر اليوم أو يوماً سابقاً' });
...
// Per-category bonus rate override — same shape as BonusRateUpdateSchema but
// keyed on the product category instead of a username.
export const CategoryBonusRateUpdateSchema = z.object({
  category:          z.string().min(1, 'الفئة مطلوبة'),
  seller_fixed:      z.preprocess(emptyToUndef, z.coerce.number().min(0, 'العمولة لا يمكن أن تكون سالبة').optional()),
  seller_percentage: z.preprocess(emptyToUndef, z.coerce.number().min(0).max(100, 'النسبة بين 0 و 100').optional()),
  driver_fixed:      z.preprocess(emptyToUndef, z.coerce.number().min(0, 'العمولة لا يمكن أن تكون سالبة').optional()),
...
// Per-user × per-category override — the most specific bonus-rate layer.
export const UserCategoryBonusRateUpdateSchema = z.object({
  username:          z.string().min(1, 'اسم المستخدم مطلوب'),
  category:          z.string().min(1, 'الفئة مطلوبة'),
  seller_fixed:      z.preprocess(emptyToUndef, z.coerce.number().min(0, 'العمولة لا يمكن أن تكون سالبة').optional()),
  seller_percentage: z.preprocess(emptyToUndef, z.coerce.number().min(0).max(100, 'النسبة بين 0 و 100').optional()),
...
export const SaleUpdateSchema = z.object({
  ...
  // Delivery address — editable while the order is still reserved.
  clientAddress: nullable(z.string().optional()),
  notes:       nullable(z.string().optional().default('')),
  downPaymentExpected: optionalNum,
});
