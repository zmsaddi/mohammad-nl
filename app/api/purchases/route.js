...
export async function GET(request) {
  ...
  try {
    const rows = await getRows(SHEETS.PURCHASES);
...
export async function POST(request) {
  ...
  try {
    ...
    const id = await getNextId(SHEETS.PURCHASES);
    const total = (parseFloat(data.quantity) || 0) * (parseFloat(data.unitPrice) || 0);

    await appendRow(SHEETS.PURCHASES, [
      id,
      data.date,
      data.supplier,
      data.item,
...
export async function DELETE(request) {
  ...
  try {
    ...
    await deleteRowById(SHEETS.PURCHASES, id);
