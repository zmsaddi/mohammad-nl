    ...
    <AppLayout>
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <>
          {/* Unified mobile-first renderer: one column spec → mobile cards
              (base) + desktop table (≥768px). No more table/card duplication. */}
          <DataView
            rows={paginatedRows}
            sort={{ requestSort, getAriaSort, getSortIndicator }}
            onRowClick={(inv) => setSelectedInvoice(inv)}
            columns={[
              { key: 'ref_code', label: 'رقم الفاتورة', sortable: true, cell: (r) => <span style={{ fontSize: '0.8rem', color: '#6366f1', fontWeight: 600 }}>{r.ref_code}</span> },
              ...
              { key: 'quantity', label: 'الكمية', sortable: true, align: 'end', cell: (r) => formatNumber(r.quantity) },
              { key: 'total', label: 'الإجمالي', sortable: true, align: 'end', cell: (r) => <strong>{formatNumber(r.total)}</strong> },
              { key: 'payment_type', label: 'الدفع', sortable: true, cell: (r) => <StatusBadge status={r.payment_type || 'كاش'} /> },
              { key: 'payment_status', label: 'حالة الدفع', sortable: true, cell: (r) => <StatusBadge status={derivePaymentStatus(r)} /> },
              { key: 'vin', label: 'VIN', sortable: true, cell: (r) => <span style={{ direction: 'ltr', unicodeBidi: 'plaintext', fontWeight: 600, color: '#4f46e5' }}>{r.vin || '-'}</span> },
            ]}
            actions={(row) => (
              <>
